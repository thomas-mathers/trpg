...
internal class BuildingGenerator {
    ...
    public BuildingGeneratorResult Generate(BuildingGeneratorInput input) {
        ...
        var props = specs.Zip(rooms)
            .SelectMany(pair => pair.First.Props.Select(p => p.Factory(pair.Second.Id, input.WorldId)))
            .ToList();
        ...
        for (var i = 0; i < roomsByFloor.Length - 1; i++) {
            ...
            props.Add(new RoomConnector {
                ...
                DestinationRoomId = roomAbove.Id, WorldId = input.WorldId
            });

            props.Add(new RoomConnector {
                RoomId = roomAbove.Id, Name = "Staircase", Description = "A staircase leading down.",
                DestinationRoomId = roomBelow.Id, WorldId = input.WorldId
            });
        }
    ...
    private static RoomSpec[] GetBakerySpecs(Guid? ownerId) {
        return [
            new RoomSpec("Bakery", "A warm room smelling of fresh bread.", 0, 6, [
                new PropSpec("Oven",
                    (id, worldId) => new Workstation {
                        RoomId = id, WorldId = worldId, Name = "Oven", Description = "A large stone oven for baking.",
