...
public class WorldGeneratorEmploymentTests {
    ...
    public void AssignEmployment_AssignsMatchingProfessionAndWorkJob_WhenSlotAvailable() {
        ...
        var context = MakeContext(eligible: [adult],
            slots: [new WorldGenerator.ShopEmploymentSlot(shopRoomId, Profession.Baker,
                WorldGenerator.StaffDayOffPatterns[1], WorkHours)],
    ...
    public void AssignEmployment_AssignsUnemployedWithSevenIndependentDays_WhenNoSlotsAvailable() {
        // Arrange
        var adult = Builders.MakeCreature(_worldId, profession: Profession.Unemployed);
        var context = MakeContext(eligible: [adult], slots: [],
            homeRooms: new Dictionary<Guid, Guid> { [adult.Id] = Guid.NewGuid() });
    ...
    public void AssignEmployment_SharesFamilyDayOff_WithHomemakerAndKids() {
        ...
        var context = MakeContext(eligible: [father],
            slots: [new WorldGenerator.ShopEmploymentSlot(shopRoomId, Profession.Baker, daysOff, WorkHours)],
        ...
        foreach (var day in daysOff) {
            var fatherJob = Assert.Single(context.Jobs, j => j.CreatureId == father.Id && j.SpecificDay == day);
            var homemakerJob = Assert.Single(context.Jobs, j => j.CreatureId == homemaker.Id && j.SpecificDay == day);
            var kidJob = Assert.Single(context.Jobs, j => j.CreatureId == kid.Id && j.SpecificDay == day);
    ...
    public void AssignEmployment_GivesSoloDayOff_WhenHouseholdHasNoHomemaker() {
        ...
        var context = MakeContext(eligible: [adult],
            slots: [new WorldGenerator.ShopEmploymentSlot(shopRoomId, Profession.Tailor, daysOff, WorkHours)],
            homeRooms: new Dictionary<Guid, Guid> { [adult.Id] = Guid.NewGuid() },
            fatherIds: [adult.Id]);
        ...
        // Assert
        Assert.Equal(daysOff.Count, context.Jobs.Count(j => j.CreatureId == adult.Id && j.SpecificDay != null));
    ...
    public void AssignEmployment_ExcludesTavernFromFamilyDay_WhenAMinorIsInTheGroup() {
        ...
        var context = MakeContext(eligible: [father],
            ...
            fatherIds: [father.Id],
            cityIdleCandidates: [
                new WorldGenerator.IdleCandidate(tavernRoomId, Guid.NewGuid(), 10, 1000, BuildingType.Tavern)
            ]);
        ...
        var fatherJobs = context.Jobs.Where(j => j.CreatureId == father.Id && j.SpecificDay != null).ToArray();
    ...
    public void AssignEmployment_SkipsCandidate_WhenCapacityIsBelowGroupHeadcount() {
        ...
        var context = MakeContext(eligible: [father],
            ...
            fatherIds: [father.Id],
            cityIdleCandidates: [
                new WorldGenerator.IdleCandidate(marketRoomId, Guid.NewGuid(), 2, 1000, BuildingType.GeneralGoods)
            ]);
    ...
    public void AssignEmployment_GivesShopOwnerTheirDayOffPattern_WithoutAnyEligibleAdults() {
        ...
        var daysOff = WorldGenerator.StaffDayOffPatterns[0];
        var context = MakeContext(eligible: [], slots: [],
    ...
    private WorldGenerator.CityEmploymentContext MakeContext(
        IReadOnlyList<Creature> eligible, IReadOnlyList<WorldGenerator.ShopEmploymentSlot> slots,
        ...
        return new WorldGenerator.CityEmploymentContext {
            ...
            HouseholdByMemberId = households?.ToDictionary(kv => kv.Key, kv => kv.Value)
                ?? eligible.ToDictionary(c => c.Id, c => new List<Creature> { c }),
            HomeRoomIdByMemberId = homeRooms?.ToDictionary(kv => kv.Key, kv => kv.Value) ?? new Dictionary<Guid, Guid>(),
