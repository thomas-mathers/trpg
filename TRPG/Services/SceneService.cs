...
{
    public async Task<SceneResult> GetScene(
        SceneQuery query,
    ...
    )
    {
        ...
        var districts = await locationService.GetAllDistrictsByCityId(
            bootstrap.CityId.Value,
            cancellationToken
        );
    ...
    {
        var roomSummary = await buildingService.GetRoomSummary(
            bootstrap.RoomId!.Value,
            cancellationToken
        );

        var props = await buildingService.GetStaticPropsByRoomId(
            bootstrap.RoomId.Value,
            cancellationToken
        );
        var connectors = await buildingService.GetConnectorsByRoomId(
            bootstrap.RoomId.Value,
            cancellationToken
        );
    ...
    {
        var buildings = await buildingService.GetAllByLocation(
            bootstrap.StateId,
            bootstrap.CityId,
    ...
    )
    {
        ...
        var nearbyPeopleRaw = await creatureService.GetAllNearby(
            creatureLocation,
            query.PlayerId,
            cancellationToken
        );
        ...
        var reputationByCreature = await reputationService.GetEffectiveReputations(
            query.PlayerId,
            nearbyCreatureIds,
    ...
    )
    {
        var destinationIds = connectors
            ...
            .ToArray();
        var destinationRoomsById = await buildingService.GetRoomsByIds(
            destinationIds,
            cancellationToken
        );
