...
internal sealed class ChatHub(IServiceProvider serviceProvider, GameSessionStore sessionStore) : Hub {
    ...
    public IAsyncEnumerable<string> ReceiveOpening(CancellationToken cancellationToken) {
        ...
        return StreamTurn(turnRunner.SendOpeningStreaming(cancellationToken), state, alwaysSendScene: true,
            cancellationToken);
    }
    ...
    public IAsyncEnumerable<string> SendChat(string message, CancellationToken cancellationToken) {
        var state = GetState();
        var turnRunner = ResolveTurnRunner(state);
        return StreamTurn(turnRunner.ProcessTurnStreaming(message, cancellationToken), state, alwaysSendScene: false,
            cancellationToken);
    }
    ...
    public IAsyncEnumerable<string> SendWait(int hours, CancellationToken cancellationToken) {
        var state = GetState();
        var turnRunner = ResolveTurnRunner(state);
        return StreamTurn(turnRunner.SendWaitStreaming(hours, cancellationToken), state, alwaysSendScene: true,
            cancellationToken);
    }

    private async IAsyncEnumerable<string> StreamTurn(IAsyncEnumerable<string> tokens, GameSessionState state,
        bool alwaysSendScene, [EnumeratorCancellation] CancellationToken cancellationToken) {
