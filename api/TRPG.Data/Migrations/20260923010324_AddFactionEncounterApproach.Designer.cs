// <auto-generated />
using System;
using System.Collections.Generic;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using TRPG.Data;

#nullable disable

namespace TRPG.Data.Migrations
{
    [DbContext(typeof(TrpgDbContext))]
    [Migration("20260923010324_AddFactionEncounterApproach")]
    partial class AddFactionEncounterApproach
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.11")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.HasPostgresExtension(modelBuilder, "pg_trgm");
            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("TRPG.Domain.Models.BookPage", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<int>("PageNumber")
                        .HasColumnType("integer")
                        .HasColumnName("page_number");

                    b.Property<string>("Text")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("text");

                    b.Property<Guid>("WorkId")
                        .HasColumnType("uuid")
                        .HasColumnName("work_id");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_book_pages");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_book_pages_world_id");

                    b.HasIndex("WorkId", "PageNumber")
                        .IsUnique()
                        .HasDatabaseName("ix_book_pages_work_id_page_number");

                    b.ToTable("book_pages", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.BookWork", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid?>("FactId")
                        .HasColumnType("uuid")
                        .HasColumnName("fact_id");

                    b.Property<int?>("FactPageNumber")
                        .HasColumnType("integer")
                        .HasColumnName("fact_page_number");

                    b.Property<int>("PageCount")
                        .HasColumnType("integer")
                        .HasColumnName("page_count");

                    b.Property<string>("SubjectName")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("subject_name");

                    b.Property<string>("SubjectType")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("subject_type");

                    b.Property<string>("Tier")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("tier");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("title");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_book_works");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_book_works_world_id");

                    b.HasIndex("WorldId", "Title")
                        .IsUnique()
                        .HasDatabaseName("ux_book_works_world_title")
                        .HasFilter("tier = 'Flavour'");

                    b.ToTable("book_works", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.Building", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("BuildingType")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("building_type");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("description");

                    b.Property<Guid>("ExteriorLocationId")
                        .HasColumnType("uuid")
                        .HasColumnName("exterior_location_id");

                    b.Property<Guid?>("FactionId")
                        .HasColumnType("uuid")
                        .HasColumnName("faction_id");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("name");

                    b.Property<string>("Premise")
                        .HasColumnType("text")
                        .HasColumnName("premise");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_buildings");

                    b.HasIndex("ExteriorLocationId")
                        .HasDatabaseName("ix_buildings_exterior_location_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_buildings_world_id");

                    b.ToTable("buildings", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.BuildingOwner", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("BuildingId")
                        .HasColumnType("uuid")
                        .HasColumnName("building_id");

                    b.Property<Guid>("OwnerId")
                        .HasColumnType("uuid")
                        .HasColumnName("owner_id");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_building_owners");

                    b.HasIndex("OwnerId")
                        .HasDatabaseName("ix_building_owners_owner_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_building_owners_world_id");

                    b.HasIndex("BuildingId", "OwnerId")
                        .IsUnique()
                        .HasDatabaseName("ix_building_owners_building_id_owner_id");

                    b.ToTable("building_owners", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.CaravanFare", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("RouteId")
                        .HasColumnType("uuid")
                        .HasColumnName("route_id");

                    b.Property<int>("TicketFeeGold")
                        .HasColumnType("integer")
                        .HasColumnName("ticket_fee_gold");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_caravan_fares");

                    b.HasIndex("RouteId")
                        .HasDatabaseName("ix_caravan_fares_route_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_caravan_fares_world_id");

                    b.ToTable("caravan_fares", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.CaravanTicket", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("CreatureId")
                        .HasColumnType("uuid")
                        .HasColumnName("creature_id");

                    b.Property<Guid>("DestinationLocationId")
                        .HasColumnType("uuid")
                        .HasColumnName("destination_location_id");

                    b.Property<Guid>("OriginStopLocationId")
                        .HasColumnType("uuid")
                        .HasColumnName("origin_stop_location_id");

                    b.Property<TimeSpan>("PurchasedAtPlaytime")
                        .HasColumnType("interval")
                        .HasColumnName("purchased_at_playtime");

                    b.Property<Guid>("RouteTravelerId")
                        .HasColumnType("uuid")
                        .HasColumnName("route_traveler_id");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_caravan_tickets");

                    b.HasIndex("CreatureId")
                        .HasDatabaseName("ix_caravan_tickets_creature_id");

                    b.HasIndex("RouteTravelerId")
                        .HasDatabaseName("ix_caravan_tickets_route_traveler_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_caravan_tickets_world_id");

                    b.ToTable("caravan_tickets", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.ChatMessage", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("MessageJson")
                        .IsRequired()
                        .HasColumnType("json")
                        .HasColumnName("message_json");

                    b.Property<int>("Ordinal")
                        .HasColumnType("integer")
                        .HasColumnName("ordinal");

                    b.Property<string>("Role")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("role");

                    b.Property<Guid>("SessionId")
                        .HasColumnType("uuid")
                        .HasColumnName("session_id");

                    b.HasKey("Id")
                        .HasName("pk_chat_messages");

                    b.HasIndex("SessionId", "Ordinal")
                        .IsUnique()
                        .HasDatabaseName("ix_chat_messages_session_id_ordinal");

                    b.HasIndex("SessionId", "Role")
                        .HasDatabaseName("ix_chat_messages_session_id_role");

                    b.ToTable("chat_messages", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.City", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("CountryId")
                        .HasColumnType("uuid")
                        .HasColumnName("country_id");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("description");

                    b.Property<bool>("IsCapital")
                        .HasColumnType("boolean")
                        .HasColumnName("is_capital");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("name");

                    b.Property<Guid>("StateId")
                        .HasColumnType("uuid")
                        .HasColumnName("state_id");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_cities");

                    b.HasIndex("StateId")
                        .IsUnique()
                        .HasDatabaseName("ix_cities_state_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_cities_world_id");

                    b.HasIndex("CountryId", "Name")
                        .IsUnique()
                        .HasDatabaseName("ix_cities_country_id_name");

                    b.ToTable("cities", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.Country", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("description");

                    b.Property<string>("DominantRace")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("dominant_race");

                    b.Property<string>("Focus")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("focus");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("name");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_countries");

                    b.HasIndex("WorldId", "Name")
                        .IsUnique()
                        .HasDatabaseName("ix_countries_world_id_name");

                    b.ToTable("countries", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.Creature", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("ActiveBuffs")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("active_buffs");

                    b.Property<string>("ActiveConditions")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("active_conditions");

                    b.Property<string>("ActiveDots")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("active_dots");

                    b.Property<string>("ActiveHots")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("active_hots");

                    b.Property<string>("Biography")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("biography");

                    b.Property<Guid>("BirthLocationId")
                        .HasColumnType("uuid")
                        .HasColumnName("birth_location_id");

                    b.Property<int>("BirthYear")
                        .HasColumnType("integer")
                        .HasColumnName("birth_year");

                    b.Property<int>("CarryingCapacity")
                        .HasColumnType("integer")
                        .HasColumnName("carrying_capacity");

                    b.Property<string>("CooldownRemainingByAbility")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("cooldown_remaining_by_ability");

                    b.Property<string>("CreatureType")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("creature_type");

                    b.Property<int>("CurrentAp")
                        .HasColumnType("integer")
                        .HasColumnName("current_ap");

                    b.Property<int>("CurrentHp")
                        .HasColumnType("integer")
                        .HasColumnName("current_hp");

                    b.Property<int>("CurrentMp")
                        .HasColumnType("integer")
                        .HasColumnName("current_mp");

                    b.Property<int>("Defense")
                        .HasColumnType("integer")
                        .HasColumnName("defense");

                    b.Property<int>("Dexterity")
                        .HasColumnType("integer")
                        .HasColumnName("dexterity");

                    b.Property<int>("Endurance")
                        .HasColumnType("integer")
                        .HasColumnName("endurance");

                    b.Property<float>("FireResistance")
                        .HasColumnType("real")
                        .HasColumnName("fire_resistance");

                    b.Property<string>("Gender")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("gender");

                    b.Property<float>("IceResistance")
                        .HasColumnType("real")
                        .HasColumnName("ice_resistance");

                    b.Property<int>("Intelligence")
                        .HasColumnType("integer")
                        .HasColumnName("intelligence");

                    b.Property<bool>("IsSneaking")
                        .HasColumnType("boolean")
                        .HasColumnName("is_sneaking");

                    b.Property<TimeSpan>("LastRegenPlaytime")
                        .HasColumnType("interval")
                        .HasColumnName("last_regen_playtime");

                    b.Property<int>("Level")
                        .HasColumnType("integer")
                        .HasColumnName("level");

                    b.Property<float>("LightningResistance")
                        .HasColumnType("real")
                        .HasColumnName("lightning_resistance");

                    b.Property<Guid>("LocationId")
                        .HasColumnType("uuid")
                        .HasColumnName("location_id");

                    b.Property<float>("MagicResistance")
                        .HasColumnType("real")
                        .HasColumnName("magic_resistance");

                    b.Property<int>("Mana")
                        .HasColumnType("integer")
                        .HasColumnName("mana");

                    b.Property<int>("MaximumAp")
                        .HasColumnType("integer")
                        .HasColumnName("maximum_ap");

                    b.Property<int>("MaximumHp")
                        .HasColumnType("integer")
                        .HasColumnName("maximum_hp");

                    b.Property<int>("MaximumMp")
                        .HasColumnType("integer")
                        .HasColumnName("maximum_mp");

                    b.Property<float>("MovementSpeed")
                        .HasColumnType("real")
                        .HasColumnName("movement_speed");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("name");

                    b.Property<int>("NaturalWeaponMaxDamage")
                        .HasColumnType("integer")
                        .HasColumnName("natural_weapon_max_damage");

                    b.Property<int>("NaturalWeaponMinDamage")
                        .HasColumnType("integer")
                        .HasColumnName("natural_weapon_min_damage");

                    b.Property<float>("PhysicalResistance")
                        .HasColumnType("real")
                        .HasColumnName("physical_resistance");

                    b.Property<Guid?>("PlayerCorpseOwnerId")
                        .HasColumnType("uuid")
                        .HasColumnName("player_corpse_owner_id");

                    b.Property<float>("PoisonResistance")
                        .HasColumnType("real")
                        .HasColumnName("poison_resistance");

                    b.Property<Guid?>("PreviousLocationId")
                        .HasColumnType("uuid")
                        .HasColumnName("previous_location_id");

                    b.Property<string>("Profession")
                        .HasColumnType("text")
                        .HasColumnName("profession");

                    b.Property<TimeSpan?>("RestedUntilPlaytime")
                        .HasColumnType("interval")
                        .HasColumnName("rested_until_playtime");

                    b.Property<Guid?>("SpawnerId")
                        .HasColumnType("uuid")
                        .HasColumnName("spawner_id");

                    b.Property<int>("Stamina")
                        .HasColumnType("integer")
                        .HasColumnName("stamina");

                    b.Property<string>("State")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("state");

                    b.Property<int>("Strength")
                        .HasColumnType("integer")
                        .HasColumnName("strength");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_creatures");

                    b.HasIndex("LocationId")
                        .HasDatabaseName("ix_creatures_location_id");

                    b.HasIndex("SpawnerId")
                        .HasDatabaseName("ix_creatures_spawner_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_creatures_world_id");

                    b.ToTable("creatures", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.CreatureJob", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Action")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("action");

                    b.Property<Guid>("CreatureId")
                        .HasColumnType("uuid")
                        .HasColumnName("creature_id");

                    b.Property<int>("EndHour")
                        .HasColumnType("integer")
                        .HasColumnName("end_hour");

                    b.Property<Guid>("LocationId")
                        .HasColumnType("uuid")
                        .HasColumnName("location_id");

                    b.Property<int>("Priority")
                        .HasColumnType("integer")
                        .HasColumnName("priority");

                    b.Property<string>("SpecificDay")
                        .HasColumnType("text")
                        .HasColumnName("specific_day");

                    b.Property<int>("StartHour")
                        .HasColumnType("integer")
                        .HasColumnName("start_hour");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_creature_jobs");

                    b.HasIndex("CreatureId")
                        .HasDatabaseName("ix_creature_jobs_creature_id");

                    b.HasIndex("LocationId")
                        .HasDatabaseName("ix_creature_jobs_location_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_creature_jobs_world_id");

                    b.ToTable("creature_jobs", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.CreatureKnowledge", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("KnowerId")
                        .HasColumnType("uuid")
                        .HasColumnName("knower_id");

                    b.Property<Guid>("SubjectId")
                        .HasColumnType("uuid")
                        .HasColumnName("subject_id");

                    b.Property<string>("SubjectType")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("subject_type");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_creature_knowledge");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_creature_knowledge_world_id");

                    b.HasIndex("KnowerId", "SubjectType")
                        .HasDatabaseName("ix_creature_knowledge_knower_id_subject_type");

                    b.ToTable("creature_knowledge", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.CreatureProfile", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Appearance")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("appearance");

                    b.Property<string>("Behavior")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("behavior");

                    b.Property<Guid>("CreatureId")
                        .HasColumnType("uuid")
                        .HasColumnName("creature_id");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("description");

                    b.Property<string>("PrivateBackground")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("private_background");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_creature_profiles");

                    b.HasIndex("CreatureId")
                        .IsUnique()
                        .HasDatabaseName("ix_creature_profiles_creature_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_creature_profiles_world_id");

                    b.ToTable("creature_profiles", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.CreatureQuest", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("CreatureId")
                        .HasColumnType("uuid")
                        .HasColumnName("creature_id");

                    b.Property<bool>("IsTracked")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true)
                        .HasColumnName("is_tracked");

                    b.Property<Guid>("QuestId")
                        .HasColumnType("uuid")
                        .HasColumnName("quest_id");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("status");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_creature_quests");

                    b.HasIndex("QuestId")
                        .HasDatabaseName("ix_creature_quests_quest_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_creature_quests_world_id");

                    b.HasIndex("CreatureId", "QuestId")
                        .IsUnique()
                        .HasDatabaseName("ix_creature_quests_creature_id_quest_id");

                    b.ToTable("creature_quests", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.CreatureQuestObjective", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<int>("Amount")
                        .HasColumnType("integer")
                        .HasColumnName("amount");

                    b.Property<Guid>("CreatureId")
                        .HasColumnType("uuid")
                        .HasColumnName("creature_id");

                    b.Property<Guid>("ObjectiveId")
                        .HasColumnType("uuid")
                        .HasColumnName("objective_id");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_creature_quest_objectives");

                    b.HasIndex("ObjectiveId")
                        .HasDatabaseName("ix_creature_quest_objectives_objective_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_creature_quest_objectives_world_id");

                    b.HasIndex("CreatureId", "ObjectiveId")
                        .IsUnique()
                        .HasDatabaseName("ix_creature_quest_objectives_creature_id_objective_id");

                    b.ToTable("creature_quest_objectives", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.CreatureSkill", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("CreatureId")
                        .HasColumnType("uuid")
                        .HasColumnName("creature_id");

                    b.Property<int>("Experience")
                        .HasColumnType("integer")
                        .HasColumnName("experience");

                    b.Property<int>("Level")
                        .HasColumnType("integer")
                        .HasColumnName("level");

                    b.Property<int>("SeedExperience")
                        .HasColumnType("integer")
                        .HasColumnName("seed_experience");

                    b.Property<string>("Skill")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("skill");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_creature_skills");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_creature_skills_world_id");

                    b.HasIndex("CreatureId", "Skill")
                        .IsUnique()
                        .HasDatabaseName("ix_creature_skills_creature_id_skill");

                    b.ToTable("creature_skills", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.CreatureSpawner", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("ArchetypeCreatureTypes")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("archetype_creature_types");

                    b.Property<Guid?>("FactionId")
                        .HasColumnType("uuid")
                        .HasColumnName("faction_id");

                    b.Property<TimeSpan>("LastSyncPlaytime")
                        .HasColumnType("interval")
                        .HasColumnName("last_sync_playtime");

                    b.Property<Guid>("LocationId")
                        .HasColumnType("uuid")
                        .HasColumnName("location_id");

                    b.Property<int>("MaxPopulation")
                        .HasColumnType("integer")
                        .HasColumnName("max_population");

                    b.Property<string>("Schedule")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("schedule");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_creature_spawners");

                    b.HasIndex("LocationId")
                        .HasDatabaseName("ix_creature_spawners_location_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_creature_spawners_world_id");

                    b.ToTable("creature_spawners", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.CreatureWeaponProficiency", b =>
                {
                    b.Property<Guid>("CreatureId")
                        .HasColumnType("uuid")
                        .HasColumnName("creature_id");

                    b.Property<string>("WeaponType")
                        .HasColumnType("text")
                        .HasColumnName("weapon_type");

                    b.Property<int>("Proficiency")
                        .HasColumnType("integer")
                        .HasColumnName("proficiency");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("CreatureId", "WeaponType")
                        .HasName("pk_creature_weapon_proficiencies");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_creature_weapon_proficiencies_world_id");

                    b.ToTable("creature_weapon_proficiencies", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.Crime", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid?>("CityId")
                        .HasColumnType("uuid")
                        .HasColumnName("city_id");

                    b.Property<Guid>("LocationId")
                        .HasColumnType("uuid")
                        .HasColumnName("location_id");

                    b.Property<DateTime>("OccurredAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("occurred_at");

                    b.Property<Guid>("PlayerId")
                        .HasColumnType("uuid")
                        .HasColumnName("player_id");

                    b.Property<string>("Resolution")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("resolution");

                    b.Property<DateTime?>("ResolvedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("resolved_at");

                    b.Property<DateTime?>("SettledAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("settled_at");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.Property<string>("crime_type")
                        .IsRequired()
                        .HasMaxLength(13)
                        .HasColumnType("character varying(13)")
                        .HasColumnName("crime_type");

                    b.HasKey("Id")
                        .HasName("pk_crimes");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_crimes_world_id");

                    b.HasIndex("WorldId", "PlayerId", "LocationId")
                        .HasDatabaseName("ix_crimes_world_id_player_id_location_id");

                    b.ToTable("crimes", (string)null);

                    b.HasDiscriminator<string>("crime_type").HasValue("Crime");

                    b.UseTphMappingStrategy();
                });

            modelBuilder.Entity("TRPG.Domain.Models.CrimeWitness", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("CreatureId")
                        .HasColumnType("uuid")
                        .HasColumnName("creature_id");

                    b.Property<Guid>("CrimeId")
                        .HasColumnType("uuid")
                        .HasColumnName("crime_id");

                    b.Property<string>("Kind")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("kind");

                    b.Property<string>("Resolution")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("resolution");

                    b.Property<DateTime?>("ResolvedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("resolved_at");

                    b.Property<DateTime>("WitnessedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("witnessed_at");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_crime_witnesses");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_crime_witnesses_world_id");

                    b.HasIndex("CrimeId", "CreatureId")
                        .IsUnique()
                        .HasDatabaseName("ix_crime_witnesses_crime_id_creature_id");

                    b.HasIndex("WorldId", "CreatureId")
                        .HasDatabaseName("ix_crime_witnesses_world_id_creature_id");

                    b.ToTable("crime_witnesses", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.District", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("CityId")
                        .HasColumnType("uuid")
                        .HasColumnName("city_id");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("description");

                    b.Property<string>("DistrictType")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("district_type");

                    b.Property<Guid>("LocationId")
                        .HasColumnType("uuid")
                        .HasColumnName("location_id");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("name");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_districts");

                    b.HasIndex("CityId")
                        .HasDatabaseName("ix_districts_city_id");

                    b.HasIndex("LocationId")
                        .IsUnique()
                        .HasDatabaseName("ix_districts_location_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_districts_world_id");

                    b.HasIndex("CityId", "DistrictType")
                        .IsUnique()
                        .HasDatabaseName("ix_districts_city_id_district_type");

                    b.ToTable("districts", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.DoorConnector", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("ConnectorId")
                        .HasColumnType("uuid")
                        .HasColumnName("connector_id");

                    b.Property<bool>("IsLocked")
                        .HasColumnType("boolean")
                        .HasColumnName("is_locked");

                    b.Property<int>("LockLevel")
                        .HasColumnType("integer")
                        .HasColumnName("lock_level");

                    b.Property<TimeSpan?>("UnlocksAtPlaytime")
                        .HasColumnType("interval")
                        .HasColumnName("unlocks_at_playtime");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_door_connectors");

                    b.HasIndex("ConnectorId")
                        .IsUnique()
                        .HasDatabaseName("ix_door_connectors_connector_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_door_connectors_world_id");

                    b.ToTable("door_connectors", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.DoorConnectorKey", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("DoorConnectorId")
                        .HasColumnType("uuid")
                        .HasColumnName("door_connector_id");

                    b.Property<Guid>("ItemId")
                        .HasColumnType("uuid")
                        .HasColumnName("item_id");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_door_connector_keys");

                    b.HasIndex("DoorConnectorId")
                        .HasDatabaseName("ix_door_connector_keys_door_connector_id");

                    b.HasIndex("ItemId")
                        .HasDatabaseName("ix_door_connector_keys_item_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_door_connector_keys_world_id");

                    b.ToTable("door_connector_keys", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.DoorConnectorLever", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("DoorConnectorId")
                        .HasColumnType("uuid")
                        .HasColumnName("door_connector_id");

                    b.Property<Guid>("LeverId")
                        .HasColumnType("uuid")
                        .HasColumnName("lever_id");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_door_connector_levers");

                    b.HasIndex("DoorConnectorId")
                        .HasDatabaseName("ix_door_connector_levers_door_connector_id");

                    b.HasIndex("LeverId")
                        .HasDatabaseName("ix_door_connector_levers_lever_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_door_connector_levers_world_id");

                    b.ToTable("door_connector_levers", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.DungeonExpedition", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("BuildingId")
                        .HasColumnType("uuid")
                        .HasColumnName("building_id");

                    b.Property<Guid>("CompanionId")
                        .HasColumnType("uuid")
                        .HasColumnName("companion_id");

                    b.Property<Guid>("CompanionLocationId")
                        .HasColumnType("uuid")
                        .HasColumnName("companion_location_id");

                    b.Property<string>("CompanionName")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("companion_name");

                    b.Property<string>("Discovery")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("discovery");

                    b.Property<Guid>("DiscoveryFactId")
                        .HasColumnType("uuid")
                        .HasColumnName("discovery_fact_id");

                    b.Property<Guid>("EntranceLocationId")
                        .HasColumnType("uuid")
                        .HasColumnName("entrance_location_id");

                    b.Property<string>("FinalExperience")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("final_experience");

                    b.Property<Guid>("JournalItemId")
                        .HasColumnType("uuid")
                        .HasColumnName("journal_item_id");

                    b.Property<Guid>("JournalWorkId")
                        .HasColumnType("uuid")
                        .HasColumnName("journal_work_id");

                    b.PrimitiveCollection<List<Guid>>("KnownRouteLocationIds")
                        .IsRequired()
                        .HasColumnType("uuid[]")
                        .HasColumnName("known_route_location_ids");

                    b.Property<string>("Purpose")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("purpose");

                    b.Property<string>("Separation")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("separation");

                    b.Property<Guid>("SurvivorId")
                        .HasColumnType("uuid")
                        .HasColumnName("survivor_id");

                    b.Property<string>("SurvivorName")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("survivor_name");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_dungeon_expeditions");

                    b.HasIndex("BuildingId")
                        .IsUnique()
                        .HasDatabaseName("ix_dungeon_expeditions_building_id");

                    b.HasIndex("JournalWorkId")
                        .IsUnique()
                        .HasDatabaseName("ix_dungeon_expeditions_journal_work_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_dungeon_expeditions_world_id");

                    b.ToTable("dungeon_expeditions", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.Encounter", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTime?>("CompletedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("completed_at");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<Guid?>("DepartureDestinationLocationId")
                        .HasColumnType("uuid")
                        .HasColumnName("departure_destination_location_id");

                    b.Property<Guid>("LocationId")
                        .HasColumnType("uuid")
                        .HasColumnName("location_id");

                    b.Property<string>("LocationName")
                        .HasColumnType("text")
                        .HasColumnName("location_name");

                    b.Property<Guid>("PlayerId")
                        .HasColumnType("uuid")
                        .HasColumnName("player_id");

                    b.Property<string>("State")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("state");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.Property<string>("encounter_type")
                        .IsRequired()
                        .HasMaxLength(13)
                        .HasColumnType("character varying(13)")
                        .HasColumnName("encounter_type");

                    b.HasKey("Id")
                        .HasName("pk_encounters");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_encounters_world_id");

                    b.HasIndex("PlayerId", "State")
                        .HasDatabaseName("ix_encounters_player_id_state");

                    b.ToTable("encounters", (string)null);

                    b.HasDiscriminator<string>("encounter_type").HasValue("Encounter");

                    b.UseTphMappingStrategy();
                });

            modelBuilder.Entity("TRPG.Domain.Models.EncounterGroup", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("FactionId")
                        .HasColumnType("uuid")
                        .HasColumnName("faction_id");

                    b.Property<Guid>("LocationId")
                        .HasColumnType("uuid")
                        .HasColumnName("location_id");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_encounter_groups");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_encounter_groups_world_id");

                    b.HasIndex("WorldId", "LocationId")
                        .HasDatabaseName("ix_encounter_groups_world_id_location_id");

                    b.ToTable("encounter_groups", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.EncounterGroupMember", b =>
                {
                    b.Property<Guid>("EncounterGroupId")
                        .HasColumnType("uuid")
                        .HasColumnName("encounter_group_id");

                    b.Property<Guid>("CreatureId")
                        .HasColumnType("uuid")
                        .HasColumnName("creature_id");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("EncounterGroupId", "CreatureId")
                        .HasName("pk_encounter_group_members");

                    b.HasIndex("CreatureId")
                        .IsUnique()
                        .HasDatabaseName("ix_encounter_group_members_creature_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_encounter_group_members_world_id");

                    b.ToTable("encounter_group_members", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.Fact", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Subject")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("subject");

                    b.Property<string>("Value")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("value");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_facts");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_facts_world_id");

                    b.ToTable("facts", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.FactDisclosureAttempt", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("FactId")
                        .HasColumnType("uuid")
                        .HasColumnName("fact_id");

                    b.Property<Guid>("NpcId")
                        .HasColumnType("uuid")
                        .HasColumnName("npc_id");

                    b.Property<Guid>("PlayerId")
                        .HasColumnType("uuid")
                        .HasColumnName("player_id");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_fact_disclosure_attempts");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_fact_disclosure_attempts_world_id");

                    b.HasIndex("PlayerId", "NpcId", "FactId")
                        .IsUnique()
                        .HasDatabaseName("ix_fact_disclosure_attempts_player_id_npc_id_fact_id");

                    b.ToTable("fact_disclosure_attempts", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.FactDisclosureLockout", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Approach")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("approach");

                    b.Property<Guid>("FactId")
                        .HasColumnType("uuid")
                        .HasColumnName("fact_id");

                    b.Property<Guid>("NpcId")
                        .HasColumnType("uuid")
                        .HasColumnName("npc_id");

                    b.Property<Guid>("PlayerId")
                        .HasColumnType("uuid")
                        .HasColumnName("player_id");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_fact_disclosure_lockouts");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_fact_disclosure_lockouts_world_id");

                    b.HasIndex("PlayerId", "NpcId", "FactId", "Approach")
                        .IsUnique()
                        .HasDatabaseName("ix_fact_disclosure_lockouts_player_id_npc_id_fact_id_approach");

                    b.ToTable("fact_disclosure_lockouts", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.Faction", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<int>("Aggression")
                        .HasColumnType("integer")
                        .HasColumnName("aggression");

                    b.Property<Guid?>("CityId")
                        .HasColumnType("uuid")
                        .HasColumnName("city_id");

                    b.Property<string>("CreatureType")
                        .HasColumnType("text")
                        .HasColumnName("creature_type");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("description");

                    b.Property<string>("EncounterApproach")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasColumnType("text")
                        .HasDefaultValue("None")
                        .HasColumnName("encounter_approach");

                    b.Property<bool>("IsCityFaction")
                        .HasColumnType("boolean")
                        .HasColumnName("is_city_faction");

                    b.Property<string>("Kind")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("kind");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("name");

                    b.Property<int>("ReputationSensitivity")
                        .HasColumnType("integer")
                        .HasColumnName("reputation_sensitivity");

                    b.Property<int>("RiskAversion")
                        .HasColumnType("integer")
                        .HasColumnName("risk_aversion");

                    b.Property<string>("Temperament")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasColumnType("text")
                        .HasDefaultValue("Authoritative")
                        .HasColumnName("temperament");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_factions");

                    b.HasIndex("WorldId", "CreatureType")
                        .HasDatabaseName("ix_factions_world_id_creature_type");

                    b.HasIndex("WorldId", "Kind")
                        .HasDatabaseName("ix_factions_world_id_kind");

                    b.HasIndex("WorldId", "Name")
                        .IsUnique()
                        .HasDatabaseName("ix_factions_world_id_name");

                    b.ToTable("factions", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.FactionMember", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("CreatureId")
                        .HasColumnType("uuid")
                        .HasColumnName("creature_id");

                    b.Property<Guid>("FactionId")
                        .HasColumnType("uuid")
                        .HasColumnName("faction_id");

                    b.Property<string>("Role")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("role");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_faction_members");

                    b.HasIndex("FactionId")
                        .HasDatabaseName("ix_faction_members_faction_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_faction_members_world_id");

                    b.HasIndex("CreatureId", "FactionId")
                        .IsUnique()
                        .HasDatabaseName("ix_faction_members_creature_id_faction_id");

                    b.ToTable("faction_members", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.FactionStanding", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("FactionId")
                        .HasColumnType("uuid")
                        .HasColumnName("faction_id");

                    b.Property<Guid>("OtherFactionId")
                        .HasColumnType("uuid")
                        .HasColumnName("other_faction_id");

                    b.Property<int>("Score")
                        .HasColumnType("integer")
                        .HasColumnName("score");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_faction_standings");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_faction_standings_world_id");

                    b.HasIndex("FactionId", "OtherFactionId")
                        .IsUnique()
                        .HasDatabaseName("ix_faction_standings_faction_id_other_faction_id");

                    b.ToTable("faction_standings", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.GameSession", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("PlayerId")
                        .HasColumnType("uuid")
                        .HasColumnName("player_id");

                    b.Property<TimeSpan>("Playtime")
                        .HasColumnType("interval")
                        .HasColumnName("playtime");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_game_sessions");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_game_sessions_world_id");

                    b.ToTable("game_sessions", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.GuardPatrolMember", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("CreatureId")
                        .HasColumnType("uuid")
                        .HasColumnName("creature_id");

                    b.Property<Guid>("RouteTravelerId")
                        .HasColumnType("uuid")
                        .HasColumnName("route_traveler_id");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_guard_patrol_members");

                    b.HasIndex("CreatureId")
                        .HasDatabaseName("ix_guard_patrol_members_creature_id");

                    b.HasIndex("RouteTravelerId")
                        .HasDatabaseName("ix_guard_patrol_members_route_traveler_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_guard_patrol_members_world_id");

                    b.ToTable("guard_patrol_members", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.Item", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<bool>("CanTrade")
                        .HasColumnType("boolean")
                        .HasColumnName("can_trade");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("description");

                    b.Property<int>("GoldValue")
                        .HasColumnType("integer")
                        .HasColumnName("gold_value");

                    b.Property<bool>("IsHidden")
                        .HasColumnType("boolean")
                        .HasColumnName("is_hidden");

                    b.Property<string>("Modifiers")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("modifiers");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("name");

                    b.Property<int>("Quantity")
                        .HasColumnType("integer")
                        .HasColumnName("quantity");

                    b.Property<int>("Weight")
                        .HasColumnType("integer")
                        .HasColumnName("weight");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.Property<string>("item_type")
                        .IsRequired()
                        .HasMaxLength(13)
                        .HasColumnType("character varying(13)")
                        .HasColumnName("item_type");

                    b.HasKey("Id")
                        .HasName("pk_items");

                    b.HasIndex("Name")
                        .HasDatabaseName("ix_items_name");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_items_world_id");

                    b.ToTable("items", (string)null);

                    b.HasDiscriminator<string>("item_type").HasValue("generic");

                    b.UseTphMappingStrategy();
                });

            modelBuilder.Entity("TRPG.Domain.Models.Location", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid?>("CityId")
                        .HasColumnType("uuid")
                        .HasColumnName("city_id");

                    b.Property<Guid>("CoarseAnchorLocationId")
                        .HasColumnType("uuid")
                        .HasColumnName("coarse_anchor_location_id");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("description");

                    b.Property<Guid?>("DistrictId")
                        .HasColumnType("uuid")
                        .HasColumnName("district_id");

                    b.Property<string>("Kind")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("kind");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("name");

                    b.Property<Guid?>("RoomId")
                        .HasColumnType("uuid")
                        .HasColumnName("room_id");

                    b.Property<Guid>("StateId")
                        .HasColumnType("uuid")
                        .HasColumnName("state_id");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_locations");

                    b.HasIndex("DistrictId")
                        .IsUnique()
                        .HasDatabaseName("ix_locations_district_id")
                        .HasFilter("room_id IS NULL");

                    b.HasIndex("RoomId")
                        .IsUnique()
                        .HasDatabaseName("ix_locations_room_id");

                    b.HasIndex("StateId")
                        .HasDatabaseName("ix_locations_state_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_locations_world_id");

                    b.ToTable("locations", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.LocationConnector", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("description");

                    b.Property<string>("DestinationLabel")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("destination_label");

                    b.Property<Guid>("DestinationLocationId")
                        .HasColumnType("uuid")
                        .HasColumnName("destination_location_id");

                    b.Property<string>("Direction")
                        .HasColumnType("text")
                        .HasColumnName("direction");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("name");

                    b.Property<Guid>("OriginLocationId")
                        .HasColumnType("uuid")
                        .HasColumnName("origin_location_id");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_location_connectors");

                    b.HasIndex("DestinationLocationId")
                        .HasDatabaseName("ix_location_connectors_destination_location_id");

                    b.HasIndex("OriginLocationId")
                        .HasDatabaseName("ix_location_connectors_origin_location_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_location_connectors_world_id");

                    b.HasIndex("OriginLocationId", "DestinationLocationId")
                        .HasDatabaseName("ix_location_connectors_origin_location_id_destination_location");

                    b.ToTable("location_connectors", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.NpcConversation", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<Guid>("NpcConversationHistoryId")
                        .HasColumnType("uuid")
                        .HasColumnName("npc_conversation_history_id");

                    b.Property<string>("Summary")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("summary");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_npc_conversations");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_npc_conversations_world_id");

                    b.HasIndex("NpcConversationHistoryId", "CreatedAt")
                        .HasDatabaseName("ix_npc_conversations_npc_conversation_history_id_created_at");

                    b.ToTable("npc_conversations", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.NpcConversationHistory", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("CreatureId")
                        .HasColumnType("uuid")
                        .HasColumnName("creature_id");

                    b.Property<string>("DurableFacts")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("durable_facts");

                    b.Property<Guid>("NpcId")
                        .HasColumnType("uuid")
                        .HasColumnName("npc_id");

                    b.Property<string>("OpenThreads")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("open_threads");

                    b.Property<string>("Summary")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("summary");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_npc_conversation_histories");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_npc_conversation_histories_world_id");

                    b.HasIndex("NpcId", "CreatureId")
                        .IsUnique()
                        .HasDatabaseName("ix_npc_conversation_histories_npc_id_creature_id");

                    b.ToTable("npc_conversation_histories", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.NpcConversationSessionState", b =>
                {
                    b.Property<Guid>("SessionId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("session_id");

                    b.Property<string>("OpenConversationCreatureIdsByName")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("open_conversation_creature_ids_by_name");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("SessionId")
                        .HasName("pk_npc_conversation_session_states");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_npc_conversation_session_states_world_id");

                    b.ToTable("npc_conversation_session_states", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.Prop", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("description");

                    b.Property<Guid>("LocationId")
                        .HasColumnType("uuid")
                        .HasColumnName("location_id");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("name");

                    b.Property<Guid?>("OwnerCreatureId")
                        .HasColumnType("uuid")
                        .HasColumnName("owner_creature_id");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.Property<string>("behavior_type")
                        .IsRequired()
                        .HasMaxLength(21)
                        .HasColumnType("text")
                        .HasColumnName("behavior_type");

                    b.HasKey("Id")
                        .HasName("pk_props");

                    b.HasIndex("LocationId")
                        .HasDatabaseName("ix_props_location_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_props_world_id");

                    b.ToTable("props", (string)null);

                    b.HasDiscriminator<string>("behavior_type").HasValue("Prop");

                    b.UseTphMappingStrategy();
                });

            modelBuilder.Entity("TRPG.Domain.Models.Quest", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid?>("ChainAntagonistFactionId")
                        .HasColumnType("uuid")
                        .HasColumnName("chain_antagonist_faction_id");

                    b.Property<Guid?>("ChainGiverFactionId")
                        .HasColumnType("uuid")
                        .HasColumnName("chain_giver_faction_id");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("description");

                    b.Property<Guid?>("ExclusiveGroupId")
                        .HasColumnType("uuid")
                        .HasColumnName("exclusive_group_id");

                    b.Property<Guid>("GiverId")
                        .HasColumnType("uuid")
                        .HasColumnName("giver_id");

                    b.Property<int>("GoldReward")
                        .HasColumnType("integer")
                        .HasColumnName("gold_reward");

                    b.Property<bool>("IsChainTerminal")
                        .HasColumnType("boolean")
                        .HasColumnName("is_chain_terminal");

                    b.PrimitiveCollection<List<Guid>>("ItemRewards")
                        .IsRequired()
                        .HasColumnType("uuid[]")
                        .HasColumnName("item_rewards");

                    b.Property<Guid?>("MembershipRewardFactionId")
                        .HasColumnType("uuid")
                        .HasColumnName("membership_reward_faction_id");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("name");

                    b.Property<Guid?>("PrerequisiteAlternativeGroupId")
                        .HasColumnType("uuid")
                        .HasColumnName("prerequisite_alternative_group_id");

                    b.PrimitiveCollection<List<Guid>>("PrerequisiteQuestIds")
                        .IsRequired()
                        .HasColumnType("uuid[]")
                        .HasColumnName("prerequisite_quest_ids");

                    b.Property<Guid?>("RequiredFactId")
                        .HasColumnType("uuid")
                        .HasColumnName("required_fact_id");

                    b.Property<Guid?>("RequiredFactionId")
                        .HasColumnType("uuid")
                        .HasColumnName("required_faction_id");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_quests");

                    b.HasIndex("ExclusiveGroupId")
                        .HasDatabaseName("ix_quests_exclusive_group_id");

                    b.HasIndex("GiverId")
                        .HasDatabaseName("ix_quests_giver_id");

                    b.HasIndex("PrerequisiteAlternativeGroupId")
                        .HasDatabaseName("ix_quests_prerequisite_alternative_group_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_quests_world_id");

                    b.ToTable("quests", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.QuestChainGenerationRequest", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<Guid>("PlayerId")
                        .HasColumnType("uuid")
                        .HasColumnName("player_id");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("status");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_quest_chain_generation_requests");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_quest_chain_generation_requests_world_id");

                    b.HasIndex("WorldId", "PlayerId", "Status")
                        .HasDatabaseName("ix_quest_chain_generation_requests_world_id_player_id_status");

                    b.ToTable("quest_chain_generation_requests", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.QuestObjective", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("description");

                    b.Property<Guid?>("LocationId")
                        .HasColumnType("uuid")
                        .HasColumnName("location_id");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("name");

                    b.Property<Guid>("QuestId")
                        .HasColumnType("uuid")
                        .HasColumnName("quest_id");

                    b.Property<int>("RequiredAmount")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(1)
                        .HasColumnName("required_amount");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.Property<string>("objective_kind")
                        .IsRequired()
                        .HasMaxLength(21)
                        .HasColumnType("character varying(21)")
                        .HasColumnName("objective_kind");

                    b.HasKey("Id")
                        .HasName("pk_quest_objectives");

                    b.HasIndex("QuestId")
                        .HasDatabaseName("ix_quest_objectives_quest_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_quest_objectives_world_id");

                    b.ToTable("quest_objectives", (string)null);

                    b.HasDiscriminator<string>("objective_kind").HasValue("QuestObjective");

                    b.UseTphMappingStrategy();
                });

            modelBuilder.Entity("TRPG.Domain.Models.QuestReputationReward", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("QuestId")
                        .HasColumnType("uuid")
                        .HasColumnName("quest_id");

                    b.Property<int>("Score")
                        .HasColumnType("integer")
                        .HasColumnName("score");

                    b.Property<Guid>("TargetId")
                        .HasColumnType("uuid")
                        .HasColumnName("target_id");

                    b.Property<string>("TargetType")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("target_type");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_quest_reputation_rewards");

                    b.HasIndex("QuestId")
                        .HasDatabaseName("ix_quest_reputation_rewards_quest_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_quest_reputation_rewards_world_id");

                    b.ToTable("quest_reputation_rewards", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.QuestSeedSchedule", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<TimeSpan>("LastSyncPlaytime")
                        .HasColumnType("interval")
                        .HasColumnName("last_sync_playtime");

                    b.Property<Guid>("LocationId")
                        .HasColumnType("uuid")
                        .HasColumnName("location_id");

                    b.Property<string>("Schedule")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("schedule");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_quest_seed_schedules");

                    b.HasIndex("LocationId")
                        .HasDatabaseName("ix_quest_seed_schedules_location_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_quest_seed_schedules_world_id");

                    b.ToTable("quest_seed_schedules", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.Relationship", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("RelationshipType")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("relationship_type");

                    b.Property<Guid>("RelativeId")
                        .HasColumnType("uuid")
                        .HasColumnName("relative_id");

                    b.Property<Guid>("SubjectId")
                        .HasColumnType("uuid")
                        .HasColumnName("subject_id");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_relationships");

                    b.HasIndex("RelativeId")
                        .HasDatabaseName("ix_relationships_relative_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_relationships_world_id");

                    b.HasIndex("SubjectId", "RelativeId", "RelationshipType")
                        .IsUnique()
                        .HasDatabaseName("ix_relationships_subject_id_relative_id_relationship_type");

                    b.ToTable("relationships", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.Reputation", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("CreatureId")
                        .HasColumnType("uuid")
                        .HasColumnName("creature_id");

                    b.Property<int>("Score")
                        .HasColumnType("integer")
                        .HasColumnName("score");

                    b.Property<Guid>("TargetId")
                        .HasColumnType("uuid")
                        .HasColumnName("target_id");

                    b.Property<string>("TargetType")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("target_type");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_reputations");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_reputations_world_id");

                    b.HasIndex("CreatureId", "TargetId", "TargetType")
                        .IsUnique()
                        .HasDatabaseName("ix_reputations_creature_id_target_id_target_type");

                    b.ToTable("reputations", null, t =>
                        {
                            t.HasCheckConstraint("ck_reputations_score", "score BETWEEN -100 AND 100");
                        });
                });

            modelBuilder.Entity("TRPG.Domain.Models.ReputationLogEntry", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<Guid>("CreatureId")
                        .HasColumnType("uuid")
                        .HasColumnName("creature_id");

                    b.Property<int>("DeltaScore")
                        .HasColumnType("integer")
                        .HasColumnName("delta_score");

                    b.Property<string>("Detail")
                        .HasColumnType("text")
                        .HasColumnName("detail");

                    b.Property<string>("Reason")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("reason");

                    b.Property<Guid>("TargetId")
                        .HasColumnType("uuid")
                        .HasColumnName("target_id");

                    b.Property<string>("TargetType")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("target_type");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_reputation_log_entries");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_reputation_log_entries_world_id");

                    b.HasIndex("CreatureId", "TargetId", "TargetType", "CreatedAt")
                        .HasDatabaseName("ix_reputation_log_entries_creature_id_target_id_target_type_cr");

                    b.ToTable("reputation_log_entries", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.RestockPolicy", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<TimeSpan>("LastSyncPlaytime")
                        .HasColumnType("interval")
                        .HasColumnName("last_sync_playtime");

                    b.Property<string>("Schedule")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("schedule");

                    b.Property<Guid>("WorkstationId")
                        .HasColumnType("uuid")
                        .HasColumnName("workstation_id");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_restock_policies");

                    b.HasIndex("WorkstationId")
                        .IsUnique()
                        .HasDatabaseName("ix_restock_policies_workstation_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_restock_policies_world_id");

                    b.ToTable("restock_policies", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.Room", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("BuildingId")
                        .HasColumnType("uuid")
                        .HasColumnName("building_id");

                    b.Property<int>("Capacity")
                        .HasColumnType("integer")
                        .HasColumnName("capacity");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("description");

                    b.Property<int>("FloorNumber")
                        .HasColumnType("integer")
                        .HasColumnName("floor_number");

                    b.Property<Guid>("LocationId")
                        .HasColumnType("uuid")
                        .HasColumnName("location_id");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("name");

                    b.Property<string>("Role")
                        .HasColumnType("text")
                        .HasColumnName("role");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_rooms");

                    b.HasIndex("BuildingId")
                        .HasDatabaseName("ix_rooms_building_id");

                    b.HasIndex("LocationId")
                        .IsUnique()
                        .HasDatabaseName("ix_rooms_location_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_rooms_world_id");

                    b.ToTable("rooms", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.RoomBooking", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<TimeSpan>("DueAtPlaytime")
                        .HasColumnType("interval")
                        .HasColumnName("due_at_playtime");

                    b.Property<Guid>("KeyItemId")
                        .HasColumnType("uuid")
                        .HasColumnName("key_item_id");

                    b.Property<Guid>("PlayerId")
                        .HasColumnType("uuid")
                        .HasColumnName("player_id");

                    b.Property<Guid>("RoomId")
                        .HasColumnType("uuid")
                        .HasColumnName("room_id");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_room_bookings");

                    b.HasIndex("PlayerId")
                        .HasDatabaseName("ix_room_bookings_player_id");

                    b.HasIndex("RoomId")
                        .HasDatabaseName("ix_room_bookings_room_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_room_bookings_world_id");

                    b.ToTable("room_bookings", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.Route", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<double>("LingerHours")
                        .HasColumnType("double precision")
                        .HasColumnName("linger_hours");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("name");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_routes");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_routes_world_id");

                    b.ToTable("routes", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.RouteStop", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<float>("DistanceToNextStop")
                        .HasColumnType("real")
                        .HasColumnName("distance_to_next_stop");

                    b.Property<Guid>("LocationId")
                        .HasColumnType("uuid")
                        .HasColumnName("location_id");

                    b.Property<Guid>("RouteId")
                        .HasColumnType("uuid")
                        .HasColumnName("route_id");

                    b.Property<int>("SequenceIndex")
                        .HasColumnType("integer")
                        .HasColumnName("sequence_index");

                    b.HasKey("Id")
                        .HasName("pk_route_stops");

                    b.HasIndex("RouteId")
                        .HasDatabaseName("ix_route_stops_route_id");

                    b.HasIndex("RouteId", "SequenceIndex")
                        .IsUnique()
                        .HasDatabaseName("ix_route_stops_route_id_sequence_index");

                    b.ToTable("route_stops", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.RouteTraveler", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Direction")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("direction");

                    b.Property<double>("PhaseOffsetHours")
                        .HasColumnType("double precision")
                        .HasColumnName("phase_offset_hours");

                    b.Property<Guid>("RouteId")
                        .HasColumnType("uuid")
                        .HasColumnName("route_id");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_route_travelers");

                    b.HasIndex("RouteId")
                        .HasDatabaseName("ix_route_travelers_route_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_route_travelers_world_id");

                    b.ToTable("route_travelers", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.State", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("CountryId")
                        .HasColumnType("uuid")
                        .HasColumnName("country_id");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("description");

                    b.Property<int>("Height")
                        .HasColumnType("integer")
                        .HasColumnName("height");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("name");

                    b.Property<int>("Width")
                        .HasColumnType("integer")
                        .HasColumnName("width");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_states");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_states_world_id");

                    b.HasIndex("CountryId", "Name")
                        .IsUnique()
                        .HasDatabaseName("ix_states_country_id_name");

                    b.ToTable("states", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.TravelConnector", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("ConnectorId")
                        .HasColumnType("uuid")
                        .HasColumnName("connector_id");

                    b.Property<float>("DangerLevel")
                        .HasColumnType("real")
                        .HasColumnName("danger_level");

                    b.Property<float>("Distance")
                        .HasColumnType("real")
                        .HasColumnName("distance");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_travel_connectors");

                    b.HasIndex("ConnectorId")
                        .IsUnique()
                        .HasDatabaseName("ix_travel_connectors_connector_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_travel_connectors_world_id");

                    b.ToTable("travel_connectors", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.WeatherState", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Condition")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("condition");

                    b.Property<TimeSpan>("NextChangePlaytime")
                        .HasColumnType("interval")
                        .HasColumnName("next_change_playtime");

                    b.Property<Guid>("StateId")
                        .HasColumnType("uuid")
                        .HasColumnName("state_id");

                    b.Property<Guid>("WorldId")
                        .HasColumnType("uuid")
                        .HasColumnName("world_id");

                    b.HasKey("Id")
                        .HasName("pk_weather_states");

                    b.HasIndex("StateId")
                        .IsUnique()
                        .HasDatabaseName("ix_weather_states_state_id");

                    b.HasIndex("WorldId")
                        .HasDatabaseName("ix_weather_states_world_id");

                    b.ToTable("weather_states", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.World", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("description");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("name");

                    b.Property<Guid?>("PlayerId")
                        .HasColumnType("uuid")
                        .HasColumnName("player_id");

                    b.Property<TimeSpan>("Playtime")
                        .HasColumnType("interval")
                        .HasColumnName("playtime");

                    b.HasKey("Id")
                        .HasName("pk_worlds");

                    b.HasIndex("Name")
                        .IsUnique()
                        .HasDatabaseName("ix_worlds_name");

                    b.ToTable("worlds", (string)null);
                });

            modelBuilder.Entity("TRPG.Domain.Models.AssaultCrime", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Crime");

                    b.PrimitiveCollection<List<Guid>>("VictimFactionIds")
                        .IsRequired()
                        .HasColumnType("uuid[]")
                        .HasColumnName("assault_victim_faction_ids");

                    b.Property<Guid>("VictimId")
                        .HasColumnType("uuid")
                        .HasColumnName("assault_victim_id");

                    b.Property<string>("VictimName")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("assault_victim_name");

                    b.ToTable("crimes", (string)null);

                    b.HasDiscriminator().HasValue("Assault");
                });

            modelBuilder.Entity("TRPG.Domain.Models.JailbreakCrime", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Crime");

                    b.Property<Guid>("BuildingId")
                        .HasColumnType("uuid")
                        .HasColumnName("jailbreak_building_id");

                    b.Property<string>("BuildingName")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("jailbreak_building_name");

                    b.Property<string>("Outcome")
                        .HasColumnType("text")
                        .HasColumnName("jailbreak_outcome");

                    b.PrimitiveCollection<List<Guid>>("OwnerFactionIds")
                        .IsRequired()
                        .HasColumnType("uuid[]")
                        .HasColumnName("jailbreak_owner_faction_ids");

                    b.ToTable("crimes", (string)null);

                    b.HasDiscriminator().HasValue("Jailbreak");
                });

            modelBuilder.Entity("TRPG.Domain.Models.KillCrime", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Crime");

                    b.PrimitiveCollection<List<Guid>>("VictimFactionIds")
                        .IsRequired()
                        .HasColumnType("uuid[]")
                        .HasColumnName("victim_faction_ids");

                    b.Property<Guid>("VictimId")
                        .HasColumnType("uuid")
                        .HasColumnName("victim_id");

                    b.Property<string>("VictimName")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("victim_name");

                    b.ToTable("crimes", (string)null);

                    b.HasDiscriminator().HasValue("Kill");
                });

            modelBuilder.Entity("TRPG.Domain.Models.LockpickingCrime", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Crime");

                    b.Property<Guid>("BuildingId")
                        .HasColumnType("uuid")
                        .HasColumnName("building_id");

                    b.Property<string>("BuildingName")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("building_name");

                    b.Property<string>("Outcome")
                        .HasColumnType("text")
                        .HasColumnName("lockpicking_outcome");

                    b.PrimitiveCollection<List<Guid>>("OwnerFactionIds")
                        .IsRequired()
                        .HasColumnType("uuid[]")
                        .HasColumnName("lockpicking_owner_faction_ids");

                    b.ToTable("crimes", (string)null);

                    b.HasDiscriminator().HasValue("Lockpicking");
                });

            modelBuilder.Entity("TRPG.Domain.Models.TheftCrime", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Crime");

                    b.Property<string>("Items")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("items");

                    b.Property<string>("Outcome")
                        .HasColumnType("text")
                        .HasColumnName("outcome");

                    b.Property<Guid>("OwnerCreatureId")
                        .HasColumnType("uuid")
                        .HasColumnName("owner_creature_id");

                    b.Property<Guid?>("OwnerFactionId")
                        .HasColumnType("uuid")
                        .HasColumnName("owner_faction_id");

                    b.Property<string>("OwnerName")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("owner_name");

                    b.Property<Guid>("SourceOwnerId")
                        .HasColumnType("uuid")
                        .HasColumnName("source_owner_id");

                    b.Property<string>("SourceOwnerType")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("source_owner_type");

                    b.ToTable("crimes", (string)null);

                    b.HasDiscriminator().HasValue("Theft");
                });

            modelBuilder.Entity("TRPG.Domain.Models.TrespassingCrime", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Crime");

                    b.Property<Guid>("BuildingId")
                        .HasColumnType("uuid")
                        .HasColumnName("trespassing_building_id");

                    b.Property<string>("BuildingName")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("trespassing_building_name");

                    b.PrimitiveCollection<List<Guid>>("OwnerFactionIds")
                        .IsRequired()
                        .HasColumnType("uuid[]")
                        .HasColumnName("trespassing_owner_faction_ids");

                    b.ToTable("crimes", (string)null);

                    b.HasDiscriminator().HasValue("Trespassing");
                });

            modelBuilder.Entity("TRPG.Domain.Models.FightEncounter", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Encounter");

                    b.PrimitiveCollection<List<Guid>>("CombatantIds")
                        .IsRequired()
                        .HasColumnType("uuid[]")
                        .HasColumnName("combatant_ids");

                    b.Property<bool>("HasSurpriseRound")
                        .HasColumnType("boolean")
                        .HasColumnName("has_surprise_round");

                    b.Property<string>("Outcome")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("outcome");

                    b.Property<int>("RoundsResolved")
                        .HasColumnType("integer")
                        .HasColumnName("rounds_resolved");

                    b.ToTable("encounters", (string)null);

                    b.HasDiscriminator().HasValue("Fight");
                });

            modelBuilder.Entity("TRPG.Domain.Models.GuardEncounter", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Encounter");

                    b.Property<Guid>("CityFactionId")
                        .HasColumnType("uuid")
                        .HasColumnName("city_faction_id");

                    b.Property<int>("FineAmount")
                        .HasColumnType("integer")
                        .HasColumnName("fine_amount");

                    b.Property<Guid>("GuardCreatureId")
                        .HasColumnType("uuid")
                        .HasColumnName("guard_creature_id");

                    b.Property<string>("GuardName")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("guard_name");

                    b.Property<int>("JailHours")
                        .HasColumnType("integer")
                        .HasColumnName("jail_hours");

                    b.Property<string>("RecentOffenses")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("recent_offenses");

                    b.Property<int>("ReputationScore")
                        .HasColumnType("integer")
                        .HasColumnName("reputation_score");

                    b.Property<Guid?>("TriggeringCrimeId")
                        .HasColumnType("uuid")
                        .HasColumnName("triggering_crime_id");

                    b.ToTable("encounters", (string)null);

                    b.HasDiscriminator().HasValue("Guard");
                });

            modelBuilder.Entity("TRPG.Domain.Models.HostileEncounter", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Encounter");

                    b.Property<Guid>("FactionId")
                        .HasColumnType("uuid")
                        .HasColumnName("faction_id");

                    b.Property<string>("FactionName")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("faction_name");

                    b.Property<string>("Members")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("members");

                    b.ToTable("encounters", (string)null);

                    b.HasDiscriminator().HasValue("Hostile");
                });

            modelBuilder.Entity("TRPG.Domain.Models.ShakedownEncounter", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Encounter");

                    b.Property<Guid>("FactionId")
                        .HasColumnType("uuid")
                        .HasColumnName("faction_id");

                    b.Property<string>("FactionName")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("faction_name");

                    b.Property<string>("Members")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("members");

                    b.Property<int>("TollAmount")
                        .HasColumnType("integer")
                        .HasColumnName("toll_amount");

                    b.ToTable("encounters", null, t =>
                        {
                            t.Property("FactionId")
                                .HasColumnName("shakedown_encounter_faction_id");

                            t.Property("FactionName")
                                .HasColumnName("shakedown_encounter_faction_name");

                            t.Property("Members")
                                .HasColumnName("shakedown_encounter_members");
                        });

                    b.HasDiscriminator().HasValue("Shakedown");
                });

            modelBuilder.Entity("TRPG.Domain.Models.SuspicionEncounter", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Encounter");

                    b.Property<string>("Cause")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("cause");

                    b.Property<Guid>("CityFactionId")
                        .HasColumnType("uuid")
                        .HasColumnName("city_faction_id");

                    b.Property<Guid>("GuardCreatureId")
                        .HasColumnType("uuid")
                        .HasColumnName("guard_creature_id");

                    b.Property<string>("GuardName")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("guard_name");

                    b.ToTable("encounters", null, t =>
                        {
                            t.Property("CityFactionId")
                                .HasColumnName("suspicion_encounter_city_faction_id");

                            t.Property("GuardCreatureId")
                                .HasColumnName("suspicion_encounter_guard_creature_id");

                            t.Property("GuardName")
                                .HasColumnName("suspicion_encounter_guard_name");
                        });

                    b.HasDiscriminator().HasValue("Suspicion");
                });

            modelBuilder.Entity("TRPG.Domain.Models.TheftEncounter", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Encounter");

                    b.Property<Guid>("ConfrontingCreatureId")
                        .HasColumnType("uuid")
                        .HasColumnName("confronting_creature_id");

                    b.Property<string>("ConfrontingName")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("confronting_name");

                    b.PrimitiveCollection<List<Guid>>("ItemIds")
                        .IsRequired()
                        .HasColumnType("uuid[]")
                        .HasColumnName("item_ids");

                    b.PrimitiveCollection<List<string>>("ItemNames")
                        .IsRequired()
                        .HasColumnType("text[]")
                        .HasColumnName("item_names");

                    b.Property<string>("ItemSelections")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("item_selections");

                    b.Property<Guid>("OwnerCreatureId")
                        .HasColumnType("uuid")
                        .HasColumnName("owner_creature_id");

                    b.Property<string>("OwnerName")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("owner_name");

                    b.Property<Guid?>("SourceOwnerId")
                        .HasColumnType("uuid")
                        .HasColumnName("source_owner_id");

                    b.Property<string>("SourceOwnerType")
                        .HasColumnType("text")
                        .HasColumnName("source_owner_type");

                    b.Property<Guid>("TheftCrimeId")
                        .HasColumnType("uuid")
                        .HasColumnName("theft_crime_id");

                    b.PrimitiveCollection<List<Guid>>("WitnessCreatureIds")
                        .IsRequired()
                        .HasColumnType("uuid[]")
                        .HasColumnName("witness_creature_ids");

                    b.ToTable("encounters", (string)null);

                    b.HasDiscriminator().HasValue("Theft");
                });

            modelBuilder.Entity("TRPG.Domain.Models.TrapEncounter", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Encounter");

                    b.Property<Guid>("TargetLocationId")
                        .HasColumnType("uuid")
                        .HasColumnName("target_location_id");

                    b.Property<string>("TargetLocationName")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("target_location_name");

                    b.Property<Guid>("TrapId")
                        .HasColumnType("uuid")
                        .HasColumnName("trap_id");

                    b.Property<string>("TrapKind")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("trap_kind");

                    b.ToTable("encounters", (string)null);

                    b.HasDiscriminator().HasValue("Trap");
                });

            modelBuilder.Entity("TRPG.Domain.Models.Accessory", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Item");

                    b.Property<int>("Level")
                        .ValueGeneratedOnUpdateSometimes()
                        .HasColumnType("integer")
                        .HasColumnName("level");

                    b.Property<string>("Rarity")
                        .IsRequired()
                        .ValueGeneratedOnUpdateSometimes()
                        .HasColumnType("text")
                        .HasColumnName("rarity");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("accessory_type");

                    b.ToTable("items", (string)null);

                    b.HasDiscriminator().HasValue("accessory");
                });

            modelBuilder.Entity("TRPG.Domain.Models.Ammunition", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Item");

                    b.Property<int>("Level")
                        .ValueGeneratedOnUpdateSometimes()
                        .HasColumnType("integer")
                        .HasColumnName("level");

                    b.Property<string>("Rarity")
                        .IsRequired()
                        .ValueGeneratedOnUpdateSometimes()
                        .HasColumnType("text")
                        .HasColumnName("rarity");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("ammo_type");

                    b.ToTable("items", (string)null);

                    b.HasDiscriminator().HasValue("ammunition");
                });

            modelBuilder.Entity("TRPG.Domain.Models.Armor", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Item");

                    b.Property<string>("ArmorClass")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("armor_class");

                    b.Property<int>("Defense")
                        .HasColumnType("integer")
                        .HasColumnName("defense");

                    b.Property<int>("DurabilityCurrent")
                        .HasColumnType("integer")
                        .HasColumnName("durability_current");

                    b.Property<int>("DurabilityMax")
                        .HasColumnType("integer")
                        .HasColumnName("durability_max");

                    b.Property<int>("Level")
                        .ValueGeneratedOnUpdateSometimes()
                        .HasColumnType("integer")
                        .HasColumnName("level");

                    b.Property<string>("Rarity")
                        .IsRequired()
                        .ValueGeneratedOnUpdateSometimes()
                        .HasColumnType("text")
                        .HasColumnName("rarity");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("armor_type");

                    b.ToTable("items", (string)null);

                    b.HasDiscriminator().HasValue("armor");
                });

            modelBuilder.Entity("TRPG.Domain.Models.Book", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Item");

                    b.Property<Guid>("WorkId")
                        .HasColumnType("uuid")
                        .HasColumnName("work_id");

                    b.ToTable("items", (string)null);

                    b.HasDiscriminator().HasValue("book");
                });

            modelBuilder.Entity("TRPG.Domain.Models.Consumable", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Item");

                    b.Property<int>("Duration")
                        .HasColumnType("integer")
                        .HasColumnName("duration");

                    b.Property<int>("Level")
                        .ValueGeneratedOnUpdateSometimes()
                        .HasColumnType("integer")
                        .HasColumnName("level");

                    b.Property<string>("Rarity")
                        .IsRequired()
                        .ValueGeneratedOnUpdateSometimes()
                        .HasColumnType("text")
                        .HasColumnName("rarity");

                    b.Property<string>("Resource")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("resource");

                    b.Property<int>("RestoreAmount")
                        .HasColumnType("integer")
                        .HasColumnName("restore_amount");

                    b.ToTable("items", (string)null);

                    b.HasDiscriminator().HasValue("consumable");
                });

            modelBuilder.Entity("TRPG.Domain.Models.Gold", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Item");

                    b.ToTable("items", (string)null);

                    b.HasDiscriminator().HasValue("gold");
                });

            modelBuilder.Entity("TRPG.Domain.Models.Key", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Item");

                    b.ToTable("items", (string)null);

                    b.HasDiscriminator().HasValue("key");
                });

            modelBuilder.Entity("TRPG.Domain.Models.Shield", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Item");

                    b.Property<float>("BlockChance")
                        .HasColumnType("real")
                        .HasColumnName("block_chance");

                    b.Property<int>("Defense")
                        .HasColumnType("integer")
                        .HasColumnName("defense");

                    b.Property<int>("DurabilityCurrent")
                        .HasColumnType("integer")
                        .HasColumnName("durability_current");

                    b.Property<int>("DurabilityMax")
                        .HasColumnType("integer")
                        .HasColumnName("durability_max");

                    b.Property<int>("Level")
                        .ValueGeneratedOnUpdateSometimes()
                        .HasColumnType("integer")
                        .HasColumnName("level");

                    b.Property<string>("Rarity")
                        .IsRequired()
                        .ValueGeneratedOnUpdateSometimes()
                        .HasColumnType("text")
                        .HasColumnName("rarity");

                    b.ToTable("items", null, t =>
                        {
                            t.Property("Defense")
                                .HasColumnName("shield_defense");

                            t.Property("DurabilityCurrent")
                                .HasColumnName("shield_durability_current");

                            t.Property("DurabilityMax")
                                .HasColumnName("shield_durability_max");
                        });

                    b.HasDiscriminator().HasValue("shield");
                });

            modelBuilder.Entity("TRPG.Domain.Models.Weapon", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Item");

                    b.Property<int>("AttacksPerTurn")
                        .HasColumnType("integer")
                        .HasColumnName("attacks_per_turn");

                    b.Property<int>("DurabilityCurrent")
                        .HasColumnType("integer")
                        .HasColumnName("durability_current");

                    b.Property<int>("DurabilityMax")
                        .HasColumnType("integer")
                        .HasColumnName("durability_max");

                    b.Property<bool>("IsTwoHanded")
                        .HasColumnType("boolean")
                        .HasColumnName("is_two_handed");

                    b.Property<int>("Level")
                        .ValueGeneratedOnUpdateSometimes()
                        .HasColumnType("integer")
                        .HasColumnName("level");

                    b.Property<int>("MaxDamage")
                        .HasColumnType("integer")
                        .HasColumnName("max_damage");

                    b.Property<int>("MinDamage")
                        .HasColumnType("integer")
                        .HasColumnName("min_damage");

                    b.Property<int>("Range")
                        .HasColumnType("integer")
                        .HasColumnName("range");

                    b.Property<string>("Rarity")
                        .IsRequired()
                        .ValueGeneratedOnUpdateSometimes()
                        .HasColumnType("text")
                        .HasColumnName("rarity");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("weapon_type");

                    b.ToTable("items", null, t =>
                        {
                            t.Property("DurabilityCurrent")
                                .HasColumnName("weapon_durability_current");

                            t.Property("DurabilityMax")
                                .HasColumnName("weapon_durability_max");
                        });

                    b.HasDiscriminator().HasValue("weapon");
                });

            modelBuilder.Entity("TRPG.Domain.Models.Bed", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Prop");

                    b.Property<Guid?>("AssignedCreatureId")
                        .HasColumnType("uuid")
                        .HasColumnName("assigned_creature_id");

                    b.Property<Guid?>("OccupantId")
                        .HasColumnType("uuid")
                        .HasColumnName("occupant_id");

                    b.ToTable("props", (string)null);

                    b.HasDiscriminator().HasValue("Bed");
                });

            modelBuilder.Entity("TRPG.Domain.Models.Cell", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Prop");

                    b.Property<Guid?>("CreatureId")
                        .HasColumnType("uuid")
                        .HasColumnName("creature_id");

                    b.Property<bool>("IsLocked")
                        .ValueGeneratedOnUpdateSometimes()
                        .HasColumnType("boolean")
                        .HasColumnName("is_locked");

                    b.Property<Guid?>("KeyItemId")
                        .ValueGeneratedOnUpdateSometimes()
                        .HasColumnType("uuid")
                        .HasColumnName("key_item_id");

                    b.Property<int>("LockLevel")
                        .ValueGeneratedOnUpdateSometimes()
                        .HasColumnType("integer")
                        .HasColumnName("lock_level");

                    b.ToTable("props", (string)null);

                    b.HasDiscriminator().HasValue("Cell");
                });

            modelBuilder.Entity("TRPG.Domain.Models.Container", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Prop");

                    b.Property<bool>("IsLocked")
                        .ValueGeneratedOnUpdateSometimes()
                        .HasColumnType("boolean")
                        .HasColumnName("is_locked");

                    b.Property<Guid?>("KeyItemId")
                        .ValueGeneratedOnUpdateSometimes()
                        .HasColumnType("uuid")
                        .HasColumnName("key_item_id");

                    b.Property<int>("LockLevel")
                        .ValueGeneratedOnUpdateSometimes()
                        .HasColumnType("integer")
                        .HasColumnName("lock_level");

                    b.Property<int?>("StorageSize")
                        .HasColumnType("integer")
                        .HasColumnName("storage_size");

                    b.ToTable("props", (string)null);

                    b.HasDiscriminator().HasValue("Container");
                });

            modelBuilder.Entity("TRPG.Domain.Models.Seat", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Prop");

                    b.Property<Guid?>("OccupantId")
                        .HasColumnType("uuid")
                        .HasColumnName("occupant_id");

                    b.ToTable("props", null, t =>
                        {
                            t.Property("OccupantId")
                                .HasColumnName("seat_occupant_id");
                        });

                    b.HasDiscriminator().HasValue("Seat");
                });

            modelBuilder.Entity("TRPG.Domain.Models.Sign", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Prop");

                    b.ToTable("props", (string)null);

                    b.HasDiscriminator().HasValue("Sign");
                });

            modelBuilder.Entity("TRPG.Domain.Models.Trap", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Prop");

                    b.Property<bool>("IsResolved")
                        .HasColumnType("boolean")
                        .HasColumnName("is_resolved");

                    b.Property<Guid?>("TargetId")
                        .HasColumnType("uuid")
                        .HasColumnName("target_id");

                    b.Property<string>("TrapKind")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("trap_kind");

                    b.ToTable("props", (string)null);

                    b.HasDiscriminator().HasValue("Trap");
                });

            modelBuilder.Entity("TRPG.Domain.Models.Trigger", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Prop");

                    b.Property<bool>("IsActivated")
                        .HasColumnType("boolean")
                        .HasColumnName("is_activated");

                    b.ToTable("props", (string)null);

                    b.HasDiscriminator().HasValue("Trigger");
                });

            modelBuilder.Entity("TRPG.Domain.Models.Workstation", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Prop");

                    b.Property<Guid?>("AssignedCreatureId")
                        .HasColumnType("uuid")
                        .HasColumnName("assigned_creature_id");

                    b.Property<Guid?>("OccupantId")
                        .HasColumnType("uuid")
                        .HasColumnName("occupant_id");

                    b.Property<string>("WorkstationType")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("workstation_type");

                    b.ToTable("props", null, t =>
                        {
                            t.Property("AssignedCreatureId")
                                .HasColumnName("workstation_assigned_creature_id");

                            t.Property("OccupantId")
                                .HasColumnName("workstation_occupant_id");
                        });

                    b.HasDiscriminator().HasValue("Workstation");
                });

            modelBuilder.Entity("TRPG.Domain.Models.ClearLocationObjective", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.QuestObjective");

                    b.Property<Guid>("BuildingId")
                        .HasColumnType("uuid")
                        .HasColumnName("building_id");

                    b.ToTable("quest_objectives", (string)null);

                    b.HasDiscriminator().HasValue("ClearLocation");
                });

            modelBuilder.Entity("TRPG.Domain.Models.CollectItemObjective", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.QuestObjective");

                    b.Property<Guid>("ItemId")
                        .HasColumnType("uuid")
                        .HasColumnName("item_id");

                    b.ToTable("quest_objectives", null, t =>
                        {
                            t.Property("ItemId")
                                .HasColumnName("collect_item_objective_item_id");
                        });

                    b.HasDiscriminator().HasValue("CollectItem");
                });

            modelBuilder.Entity("TRPG.Domain.Models.DeliverItemObjective", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.QuestObjective");

                    b.Property<Guid>("ItemId")
                        .HasColumnType("uuid")
                        .HasColumnName("item_id");

                    b.Property<Guid>("RecipientId")
                        .HasColumnType("uuid")
                        .HasColumnName("recipient_id");

                    b.ToTable("quest_objectives", null, t =>
                        {
                            t.Property("RecipientId")
                                .HasColumnName("deliver_item_objective_recipient_id");
                        });

                    b.HasDiscriminator().HasValue("DeliverItem");
                });

            modelBuilder.Entity("TRPG.Domain.Models.ExploreLocationObjective", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.QuestObjective");

                    b.ToTable("quest_objectives", (string)null);

                    b.HasDiscriminator().HasValue("ExploreLocation");
                });

            modelBuilder.Entity("TRPG.Domain.Models.FreeCreatureObjective", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.QuestObjective");

                    b.Property<Guid>("CreatureId")
                        .HasColumnType("uuid")
                        .HasColumnName("creature_id");

                    b.ToTable("quest_objectives", null, t =>
                        {
                            t.Property("CreatureId")
                                .HasColumnName("free_creature_objective_creature_id");
                        });

                    b.HasDiscriminator().HasValue("FreeCreature");
                });

            modelBuilder.Entity("TRPG.Domain.Models.GiveItemKindObjective", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.QuestObjective");

                    b.Property<string>("ItemName")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("item_name");

                    b.Property<Guid>("RecipientId")
                        .HasColumnType("uuid")
                        .HasColumnName("recipient_id");

                    b.ToTable("quest_objectives", null, t =>
                        {
                            t.Property("RecipientId")
                                .HasColumnName("give_item_kind_objective_recipient_id");
                        });

                    b.HasDiscriminator().HasValue("GiveItemKind");
                });

            modelBuilder.Entity("TRPG.Domain.Models.GiveItemsObjective", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.QuestObjective");

                    b.PrimitiveCollection<List<Guid>>("ItemIds")
                        .IsRequired()
                        .HasColumnType("uuid[]")
                        .HasColumnName("item_ids");

                    b.Property<Guid>("RecipientId")
                        .HasColumnType("uuid")
                        .HasColumnName("recipient_id");

                    b.ToTable("quest_objectives", (string)null);

                    b.HasDiscriminator().HasValue("GiveItem");
                });

            modelBuilder.Entity("TRPG.Domain.Models.InteractWithPropObjective", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.QuestObjective");

                    b.Property<Guid>("TriggerId")
                        .HasColumnType("uuid")
                        .HasColumnName("trigger_id");

                    b.ToTable("quest_objectives", (string)null);

                    b.HasDiscriminator().HasValue("InteractWithProp");
                });

            modelBuilder.Entity("TRPG.Domain.Models.KillCreatureObjective", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.QuestObjective");

                    b.Property<Guid>("CreatureId")
                        .HasColumnType("uuid")
                        .HasColumnName("creature_id");

                    b.ToTable("quest_objectives", null, t =>
                        {
                            t.Property("CreatureId")
                                .HasColumnName("kill_creature_objective_creature_id");
                        });

                    b.HasDiscriminator().HasValue("KillCreature");
                });

            modelBuilder.Entity("TRPG.Domain.Models.KillCreatureTypeObjective", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.QuestObjective");

                    b.Property<string>("CreatureType")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("creature_type");

                    b.ToTable("quest_objectives", (string)null);

                    b.HasDiscriminator().HasValue("KillCreatureType");
                });

            modelBuilder.Entity("TRPG.Domain.Models.LearnFactFromCreatureObjective", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.QuestObjective");

                    b.Property<int>("BaseWillingness")
                        .HasColumnType("integer")
                        .HasColumnName("base_willingness");

                    b.Property<int>("BribeWillingness")
                        .HasColumnType("integer")
                        .HasColumnName("bribe_willingness");

                    b.Property<Guid>("CreatureId")
                        .HasColumnType("uuid")
                        .HasColumnName("creature_id");

                    b.Property<Guid>("FactId")
                        .HasColumnType("uuid")
                        .HasColumnName("fact_id");

                    b.Property<int>("IntimidationWillingness")
                        .HasColumnType("integer")
                        .HasColumnName("intimidation_willingness");

                    b.Property<Guid?>("ReasonFactId")
                        .HasColumnType("uuid")
                        .HasColumnName("reason_fact_id");

                    b.PrimitiveCollection<List<Guid>>("RequiredSupportingQuestIds")
                        .IsRequired()
                        .HasColumnType("uuid[]")
                        .HasColumnName("required_supporting_quest_ids");

                    b.ToTable("quest_objectives", (string)null);

                    b.HasDiscriminator().HasValue("LearnFactFromCreature");
                });

            modelBuilder.Entity("TRPG.Domain.Models.ReportFactToCreatureObjective", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.QuestObjective");

                    b.Property<Guid>("CreatureId")
                        .HasColumnType("uuid")
                        .HasColumnName("creature_id");

                    b.Property<Guid>("FactId")
                        .HasColumnType("uuid")
                        .HasColumnName("fact_id");

                    b.ToTable("quest_objectives", null, t =>
                        {
                            t.Property("CreatureId")
                                .HasColumnName("report_fact_to_creature_objective_creature_id");

                            t.Property("FactId")
                                .HasColumnName("report_fact_to_creature_objective_fact_id");
                        });

                    b.HasDiscriminator().HasValue("ReportFactToCreature");
                });

            modelBuilder.Entity("TRPG.Domain.Models.CaravanScheduleSign", b =>
                {
                    b.HasBaseType("TRPG.Domain.Models.Sign");

                    b.ToTable("props", (string)null);

                    b.HasDiscriminator().HasValue("CaravanScheduleSign");
                });

            modelBuilder.Entity("TRPG.Domain.Models.Country", b =>
                {
                    b.OwnsOne("TRPG.Domain.Models.Polygon", "Boundary", b1 =>
                        {
                            b1.Property<Guid>("CountryId");

                            b1.HasKey("CountryId")
                                .HasName("pk_countries");

                            b1.ToTable("countries");

                            b1
                                .ToJson("boundary")
                                .HasColumnType("jsonb");

                            b1.WithOwner()
                                .HasForeignKey("CountryId")
                                .HasConstraintName("fk_countries_countries_country_id");

                            b1.OwnsMany("TRPG.Domain.Models.Point", "Points", b2 =>
                                {
                                    b2.Property<Guid>("PolygonCountryId");

                                    b2.Property<int>("__synthesizedOrdinal")
                                        .ValueGeneratedOnAdd();

                                    b2.Property<double>("X");

                                    b2.Property<double>("Y");

                                    b2.HasKey("PolygonCountryId", "__synthesizedOrdinal")
                                        .HasName("pk_countries");

                                    b2.ToTable("countries");

                                    b2.WithOwner()
                                        .HasForeignKey("PolygonCountryId")
                                        .HasConstraintName("fk_countries_countries_polygon_country_id");
                                });

                            b1.Navigation("Points");
                        });

                    b.Navigation("Boundary")
                        .IsRequired();
                });

            modelBuilder.Entity("TRPG.Domain.Models.Creature", b =>
                {
                    b.OwnsOne("TRPG.Domain.Models.Attributes", "BaseAttributes", b1 =>
                        {
                            b1.Property<Guid>("CreatureId");

                            b1.Property<int>("CarryingCapacity");

                            b1.Property<int>("Defense");

                            b1.Property<int>("Dexterity");

                            b1.Property<int>("Endurance");

                            b1.Property<float>("FireResistance");

                            b1.Property<float>("IceResistance");

                            b1.Property<int>("Intelligence");

                            b1.Property<float>("LightningResistance");

                            b1.Property<float>("MagicResistance");

                            b1.Property<int>("Mana");

                            b1.Property<int>("MaximumAp");

                            b1.Property<int>("MaximumHp");

                            b1.Property<int>("MaximumMp");

                            b1.Property<float>("MovementSpeed");

                            b1.Property<float>("PhysicalResistance");

                            b1.Property<float>("PoisonResistance");

                            b1.Property<int>("Stamina");

                            b1.Property<int>("Strength");

                            b1.HasKey("CreatureId");

                            b1.ToTable("creatures");

                            b1
                                .ToJson("base_attributes")
                                .HasColumnType("jsonb");

                            b1.WithOwner()
                                .HasForeignKey("CreatureId")
                                .HasConstraintName("fk_creatures_creatures_id");
                        });

                    b.Navigation("BaseAttributes")
                        .IsRequired();
                });

            modelBuilder.Entity("TRPG.Domain.Models.CreatureQuest", b =>
                {
                    b.HasOne("TRPG.Domain.Models.Quest", "Quest")
                        .WithMany()
                        .HasForeignKey("QuestId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_creature_quests_quests_quest_id");

                    b.Navigation("Quest");
                });

            modelBuilder.Entity("TRPG.Domain.Models.CreatureQuestObjective", b =>
                {
                    b.HasOne("TRPG.Domain.Models.QuestObjective", "Objective")
                        .WithMany()
                        .HasForeignKey("ObjectiveId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_creature_quest_objectives_quest_objectives_objective_id");

                    b.Navigation("Objective");
                });

            modelBuilder.Entity("TRPG.Domain.Models.Item", b =>
                {
                    b.OwnsOne("TRPG.Domain.Models.ItemOwnership", "Ownership", b1 =>
                        {
                            b1.Property<Guid>("ItemId")
                                .HasColumnType("uuid")
                                .HasColumnName("id");

                            b1.Property<DateTime>("AcquiredAt")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("ownership_acquired_at");

                            b1.Property<string>("EquippedSlot")
                                .HasColumnType("text")
                                .HasColumnName("ownership_equipped_slot");

                            b1.Property<Guid>("OwnerId")
                                .HasColumnType("uuid")
                                .HasColumnName("ownership_owner_id");

                            b1.Property<string>("OwnerType")
                                .IsRequired()
                                .HasColumnType("text")
                                .HasColumnName("ownership_owner_type");

                            b1.HasKey("ItemId");

                            b1.HasIndex("OwnerId")
                                .IsUnique()
                                .HasDatabaseName("ux_items_gold_owner")
                                .HasFilter("item_type = 'gold'");

                            b1.HasIndex("OwnerId", "EquippedSlot")
                                .IsUnique()
                                .HasDatabaseName("ux_items_owner_equipped_slot")
                                .HasFilter("ownership_equipped_slot IS NOT NULL");

                            b1.ToTable("items");

                            b1.WithOwner()
                                .HasForeignKey("ItemId")
                                .HasConstraintName("fk_items_items_id");
                        });

                    b.Navigation("Ownership")
                        .IsRequired();
                });

            modelBuilder.Entity("TRPG.Domain.Models.LocationConnector", b =>
                {
                    b.OwnsOne("TRPG.Domain.Models.Polyline", "Path", b1 =>
                        {
                            b1.Property<Guid>("LocationConnectorId");

                            b1.HasKey("LocationConnectorId");

                            b1.ToTable("location_connectors");

                            b1
                                .ToJson("path")
                                .HasColumnType("jsonb");

                            b1.WithOwner()
                                .HasForeignKey("LocationConnectorId")
                                .HasConstraintName("fk_location_connectors_location_connectors_id");

                            b1.OwnsMany("TRPG.Domain.Models.Point", "Points", b2 =>
                                {
                                    b2.Property<Guid>("PolylineLocationConnectorId");

                                    b2.Property<int>("__synthesizedOrdinal")
                                        .ValueGeneratedOnAdd();

                                    b2.Property<double>("X");

                                    b2.Property<double>("Y");

                                    b2.HasKey("PolylineLocationConnectorId", "__synthesizedOrdinal");

                                    b2.ToTable("location_connectors");

                                    b2.WithOwner()
                                        .HasForeignKey("PolylineLocationConnectorId")
                                        .HasConstraintName("fk_location_connectors_location_connectors_polyline_location_con");
                                });

                            b1.Navigation("Points");
                        });

                    b.Navigation("Path");
                });

            modelBuilder.Entity("TRPG.Domain.Models.QuestReputationReward", b =>
                {
                    b.HasOne("TRPG.Domain.Models.Quest", null)
                        .WithMany("ReputationRewards")
                        .HasForeignKey("QuestId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_quest_reputation_rewards_quests_quest_id");
                });

            modelBuilder.Entity("TRPG.Domain.Models.Room", b =>
                {
                    b.OwnsOne("TRPG.Domain.Models.Rectangle", "Bounds", b1 =>
                        {
                            b1.Property<Guid>("RoomId");

                            b1.Property<int>("Bottom");

                            b1.Property<int>("Left");

                            b1.Property<int>("Right");

                            b1.Property<int>("Top");

                            b1.HasKey("RoomId")
                                .HasName("pk_rooms");

                            b1.ToTable("rooms");

                            b1
                                .ToJson("bounds")
                                .HasColumnType("jsonb");

                            b1.WithOwner()
                                .HasForeignKey("RoomId")
                                .HasConstraintName("fk_rooms_rooms_room_id");
                        });

                    b.Navigation("Bounds");
                });

            modelBuilder.Entity("TRPG.Domain.Models.State", b =>
                {
                    b.OwnsOne("TRPG.Domain.Models.Polygon", "Boundary", b1 =>
                        {
                            b1.Property<Guid>("StateId");

                            b1.HasKey("StateId");

                            b1.ToTable("states");

                            b1
                                .ToJson("boundary")
                                .HasColumnType("jsonb");

                            b1.WithOwner()
                                .HasForeignKey("StateId")
                                .HasConstraintName("fk_states_states_id");

                            b1.OwnsMany("TRPG.Domain.Models.Point", "Points", b2 =>
                                {
                                    b2.Property<Guid>("PolygonStateId");

                                    b2.Property<int>("__synthesizedOrdinal")
                                        .ValueGeneratedOnAdd();

                                    b2.Property<double>("X");

                                    b2.Property<double>("Y");

                                    b2.HasKey("PolygonStateId", "__synthesizedOrdinal");

                                    b2.ToTable("states");

                                    b2.WithOwner()
                                        .HasForeignKey("PolygonStateId")
                                        .HasConstraintName("fk_states_states_polygon_state_id");
                                });

                            b1.Navigation("Points");
                        });

                    b.OwnsOne("TRPG.Domain.Models.Point", "Center", b1 =>
                        {
                            b1.Property<Guid>("StateId");

                            b1.Property<double>("X");

                            b1.Property<double>("Y");

                            b1.HasKey("StateId");

                            b1.ToTable("states");

                            b1
                                .ToJson("center")
                                .HasColumnType("jsonb");

                            b1.WithOwner()
                                .HasForeignKey("StateId")
                                .HasConstraintName("fk_states_states_id");
                        });

                    b.Navigation("Boundary")
                        .IsRequired();

                    b.Navigation("Center")
                        .IsRequired();
                });

            modelBuilder.Entity("TRPG.Domain.Models.World", b =>
                {
                    b.OwnsOne("TRPG.Domain.Models.Rectangle", "Boundary", b1 =>
                        {
                            b1.Property<Guid>("WorldId");

                            b1.Property<int>("Bottom");

                            b1.Property<int>("Left");

                            b1.Property<int>("Right");

                            b1.Property<int>("Top");

                            b1.HasKey("WorldId");

                            b1.ToTable("worlds");

                            b1
                                .ToJson("boundary")
                                .HasColumnType("jsonb");

                            b1.WithOwner()
                                .HasForeignKey("WorldId")
                                .HasConstraintName("fk_worlds_worlds_id");
                        });

                    b.Navigation("Boundary")
                        .IsRequired();
                });

            modelBuilder.Entity("TRPG.Domain.Models.LearnFactFromCreatureObjective", b =>
                {
                    b.OwnsMany("TRPG.Domain.Models.SupportingFactQuestWeight", "WeightedSupportingQuestIds", b1 =>
                        {
                            b1.Property<Guid>("LearnFactFromCreatureObjectiveId");

                            b1.Property<int>("__synthesizedOrdinal")
                                .ValueGeneratedOnAdd();

                            b1.Property<Guid>("QuestId");

                            b1.Property<int>("Weight");

                            b1.HasKey("LearnFactFromCreatureObjectiveId", "__synthesizedOrdinal");

                            b1.ToTable("quest_objectives");

                            b1
                                .ToJson("weighted_supporting_quest_ids")
                                .HasColumnType("jsonb");

                            b1.WithOwner()
                                .HasForeignKey("LearnFactFromCreatureObjectiveId")
                                .HasConstraintName("fk_quest_objectives_quest_objectives_learn_fact_from_creature_obje");
                        });

                    b.Navigation("WeightedSupportingQuestIds");
                });

            modelBuilder.Entity("TRPG.Domain.Models.Quest", b =>
                {
                    b.Navigation("ReputationRewards");
                });
#pragma warning restore 612, 618
        }
    }
}
