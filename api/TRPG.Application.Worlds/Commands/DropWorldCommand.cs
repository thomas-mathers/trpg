...
internal class DropWorldCommandHandler(TrpgDbContext context) : ICommandHandler<DropWorldCommand>
{
    ...
    )
    {
        ...
        await context
            .DoorConnectorLevers.Where(x => x.WorldId == worldId)
            .ExecuteDeleteAsync(cancellationToken);
        ...
        await context.Facts.Where(x => x.WorldId == worldId).ExecuteDeleteAsync(cancellationToken);
        ...
        await context
            .FactionStandings.Where(x => x.WorldId == worldId)
            .ExecuteDeleteAsync(cancellationToken);
        ...
        await context
            .FactDisclosureAttempts.Where(x => x.WorldId == worldId)
            .ExecuteDeleteAsync(cancellationToken);
