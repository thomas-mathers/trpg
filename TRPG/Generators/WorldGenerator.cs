...
internal class WorldGenerator(
    ...
    private static readonly BuildingType[] StandardBuildingTypes = [
        ...
        BuildingType.GuildHall, BuildingType.Inn, BuildingType.Jail, BuildingType.Jeweler, BuildingType.Library,
        BuildingType.Stable, BuildingType.Tailor, BuildingType.Tavern, BuildingType.Temple
    ];
    ...
    internal static readonly JobAction[] DayOffActivities =
        [JobAction.Idle, JobAction.Study, JobAction.Pray, JobAction.Train, JobAction.Sit];

    private static readonly DayOfWeek[] AllWeekdays = Enum.GetValues<DayOfWeek>();

    internal record ShopEmploymentSlot(Guid RoomId, Profession EmployeeProfession, IReadOnlyList<DayOfWeek> DaysOff,
        HourWindow WorkHours, HourWindow? SleepHours = null);

    internal record StaffDayOff(Guid CreatureId, IReadOnlyList<DayOfWeek> DaysOff, HourWindow WorkHours);
    ...
    private record DayOffNeed(IReadOnlyList<Guid> ParticipantIds, Guid HomeRoomId, JobAction Action,
        HourWindow Hours, bool ExcludeTavern, bool IsUnemployedActivity);
    ...
    public async Task<WorldGeneratorResult> Generate(
        ...
        foreach (var city in geography.Cities) {
            ...
            var householdContext = new HouseholdGenerationContext {
                WorldId = worldId, City = city, StateId = state.Id, ResidentialDistrict = residentialDistrict,
                DominantRace = dominantRace, GeneratorInput = generatorInput, UsedBuildingNames = usedBuildingNames,
                CityFactionId = cityFaction.Id, CityBuildings = cityBuildings, Creatures = creatures,
                FactionMembers = factionMembers, Items = items, InventoryItems = inventoryItems, Skills = skills,
                Abilities = abilities, Jobs = jobs, BuildingOwners = buildingOwners, Rooms = rooms, Props = props,
                Relationships = relationships, RoomConnectorKeys = roomConnectorKeys,
                HouseholdByMemberId = householdByMemberId, HomeRoomIdByMemberId = homeRoomIdByMemberId,
                FatherIds = fatherIds, EligibleForEmployment = eligibleForEmployment
            };
            ...
            foreach (var type in StandardBuildingTypes) {
                ...
                var owner = GenerateAndRegisterHousehold(householdContext, GetProfessionForBuilding(type))!;
                ...
                if (type is not (BuildingType.Jail or BuildingType.Castle or BuildingType.Barracks
                ...
                if (type == BuildingType.GuildHall) {
                    ...
                    if (guildFactionId != null) {
                        ...
                        factionMembers.Add(new FactionMember {
                            FactionId = guildFactionId.Value, CreatureId = owner.Id,
                ...
                if (type == BuildingType.Inn) {
                    GenerateInnStaffing(state.Id, worldId, owner.Id, groundFloorRoomId, jobs, shopOwnerAssignments,
                        openShopSlots);
                }
                else if (type == BuildingType.GuildHall) {
                    jobs.Add(JobGenerator.GenerateWork(state.Id, owner.Id, groundFloorRoomId, worldId));
                }
                else {
                    var workHours = GetWorkHoursForBuilding(type);
                    var sleepHours = GetSleepHoursForBuilding(type);
                    jobs.Add(JobGenerator.GenerateWork(state.Id, owner.Id, groundFloorRoomId, worldId, workHours));
                    ApplySleepOverride(owner.Id, sleepHours, state.Id, worldId, jobs);

                    var staffableWorkstationCount = Math.Max(1, buildingResult.Props.OfType<Workstation>()
                        .Count(w => w.WorkstationType != WorkstationType.Reading));

                    if (staffableWorkstationCount == 1) {
                        shopOwnerAssignments.Add(new StaffDayOff(owner.Id, NonOverlappingDayOffPatterns[0],
                            workHours));
                        openShopSlots.Add(new ShopEmploymentSlot(groundFloorRoomId,
                            GetEmployeeProfessionForBuilding(type), NonOverlappingDayOffPatterns[1], workHours,
                            sleepHours));
                    }
                    else {
                        var totalStaff = Math.Min(staffableWorkstationCount, MaxShopStaff);
                        shopOwnerAssignments.Add(new StaffDayOff(owner.Id, StaffDayOffPatterns[0], workHours));
                        for (var position = 1; position < totalStaff; position++) {
                            openShopSlots.Add(new ShopEmploymentSlot(groundFloorRoomId,
                                GetEmployeeProfessionForBuilding(type), StaffDayOffPatterns[position], workHours,
                                sleepHours));
                        }
                    }
                }
                ...
                buildingOwners.Add(new BuildingOwner {
                    BuildingId = buildingResult.Building.Id, OwnerId = owner.Id, WorldId = worldId
                });
            ...
            AssignEmployment(new CityEmploymentContext {
        ...
        BiographyGenerator.AssignBiographies(
            new BiographyGeneratorInput(creatures, stateById, factionMembers, factions, relationships, jobs, rooms,
                buildings, buildingOwners));
    ...
    internal Creature? GenerateAndRegisterHousehold(HouseholdGenerationContext context,
        Profession? designatedProfession) {
        var householdResult = Random.Shared.NextDouble() < FamilyUnitChance
            ? GenerateFamilyHousehold(context.DominantRace, context.WorldId, context.StateId, context.GeneratorInput)
        ...
        var designatedWorker = designatedProfession != null
            ? household.Count >= 2 ? household[1] : household[0]
            : null;
        ...
        var houseName = SettlementNameGenerator.GenerateBuildingName(context.DominantRace, BuildingType.House,
            context.UsedBuildingNames);
        var houseResult = buildingGenerator.Generate(
            new BuildingGeneratorInput(context.StateId, context.City.Id, context.ResidentialDistrict.Id,
                houseOwner.Creature.Id, BuildingType.House, context.WorldId) {
                Name = houseName,
        ...
        foreach (var resident in household) {
            var houseKeyItem = new Item {
                WorldId = context.WorldId, Name = $"Key to {houseResult.Building.Name}",
                Description = $"A key that unlocks {houseResult.Building.Name}."
            };
            context.Items.Add(houseKeyItem);
            context.InventoryItems.Add(new InventoryItem {
                CreatureId = resident.Creature.Id, ItemId = houseKeyItem.Id, Quantity = 1, WorldId = context.WorldId
            });
            context.RoomConnectorKeys.Add(new RoomConnectorKey
                { ItemId = houseKeyItem.Id, RoomConnectorId = houseFrontDoor.Id, WorldId = context.WorldId });
        }
        ...
        foreach (var member in household) {
            ...
            context.FactionMembers.Add(new FactionMember {
                FactionId = context.CityFactionId, CreatureId = member.Creature.Id, Role = FactionRole.Member,
            ...
            context.Jobs.Add(JobGenerator.GenerateSleep(context.StateId, member.Creature.Id, memberBedRoomId,
                context.WorldId));
            ...
            context.Jobs.Add(JobGenerator.GenerateIdle(context.StateId, member.Creature.Id, homeRoomId,
                context.WorldId));
        ...
        context.BuildingOwners.Add(new BuildingOwner
            { BuildingId = houseResult.Building.Id, OwnerId = houseOwner.Creature.Id, WorldId = context.WorldId });
    ...
    internal HouseholdResult GenerateFamilyHousehold(CreatureType dominantRace, Guid worldId, Guid stateId,
        WorldGeneratorInput generatorInput) {
        ...
        var mother = creatureGenerator.Generate(new CreatureGeneratorInput(creatureType, Profession.Unemployed,
            worldId, stateId, stateId) {
        ...
        var father = creatureGenerator.Generate(new CreatureGeneratorInput(creatureType, Profession.Unemployed,
            worldId, stateId, stateId) {
            Gender = Gender.Male,
        ...
        for (var k = 0; k < kidCount; k++) {
            ...
            var kid = creatureGenerator.Generate(new CreatureGeneratorInput(creatureType, Profession.Unemployed,
                worldId, stateId, stateId) {
                Gender = kidGender,
            ...
            kid.Creature.Profession = GameClock.EpochYear - kid.Creature.BirthYear < AdultAge ? null : Profession.Unemployed;
            kids.Add(kid);
        }
    ...
    internal HouseholdResult GenerateSingleHousehold(CreatureType dominantRace, Guid worldId, Guid stateId) {
        var creatureType = PickCreatureType(dominantRace);
        var member = creatureGenerator.Generate(
            new CreatureGeneratorInput(creatureType, Profession.Unemployed, worldId, stateId, stateId) {
                MaxBirthYear = MaxAdultBirthYear
            }
        );
    ...
    private static Profession GetProfessionForBuilding(BuildingType type) {
        return type switch {
            ...
            _ => throw new ArgumentOutOfRangeException(nameof(type), type, "No profession mapped for this building type.")
        };
    }
    ...
    private static HourWindow GetWorkHoursForBuilding(BuildingType type) {
        return type switch {
            ...
            BuildingType.Blacksmith or BuildingType.Carpenter or BuildingType.Stable => new HourWindow(7, 17),
            BuildingType.ArcaneShop or BuildingType.Library => new HourWindow(9, 19),
            BuildingType.Castle or BuildingType.Jail or BuildingType.Barracks => new HourWindow(8, 20),
    ...
    private static void GenerateInnStaffing(Guid stateId, Guid worldId, Guid ownerId, Guid groundFloorRoomId,
        List<Job> jobs, List<StaffDayOff> shopOwnerAssignments, List<ShopEmploymentSlot> openShopSlots) {
        var dayShiftHours = new HourWindow(6, 18);
        var nightShiftHours = new HourWindow(18, 6);
        var nightShiftSleepHours = new HourWindow(6, 14);

        jobs.Add(JobGenerator.GenerateWork(stateId, ownerId, groundFloorRoomId, worldId, dayShiftHours));

        shopOwnerAssignments.Add(new StaffDayOff(ownerId, NonOverlappingDayOffPatterns[0], dayShiftHours));

        openShopSlots.Add(new ShopEmploymentSlot(groundFloorRoomId, Profession.Innkeeper,
            NonOverlappingDayOffPatterns[1], dayShiftHours));
        openShopSlots.Add(new ShopEmploymentSlot(groundFloorRoomId, Profession.Innkeeper,
            NonOverlappingDayOffPatterns[0], nightShiftHours, nightShiftSleepHours));
        openShopSlots.Add(new ShopEmploymentSlot(groundFloorRoomId, Profession.Innkeeper,
            NonOverlappingDayOffPatterns[1], nightShiftHours, nightShiftSleepHours));
    }
    ...
    internal static void AssignEmployment(CityEmploymentContext context) {
        ...
        foreach (var adult in context.EligibleForEmployment) {
            ...
            context.Jobs.Add(JobGenerator.GenerateWork(context.StateId, adult.Id, slot.RoomId, context.WorldId,
                slot.WorkHours));
            ApplySleepOverride(adult.Id, slot.SleepHours, context.StateId, context.WorldId, context.Jobs);
        ...
        foreach (var ownerAssignment in context.ShopOwnerAssignments) {
            RegisterDayOff(ownerAssignment.CreatureId, ownerAssignment.DaysOff, ownerAssignment.WorkHours,
                needsByDay, context);
        }
    ...
    // window instead.
    private static void ApplySleepOverride(Guid creatureId, HourWindow? sleepHours, Guid stateId, Guid worldId,
        List<Job> jobs) {
        if (sleepHours == null) {
            return;
        }

        var existingSleep = jobs.First(j => j.CreatureId == creatureId && j.Action == JobAction.Sleep);
        jobs.Remove(existingSleep);
        jobs.Add(JobGenerator.GenerateSleep(stateId, creatureId, existingSleep.RoomId!.Value, worldId, sleepHours));
    }
    ...
    private static void RegisterDayOff(Guid adultId, IReadOnlyList<DayOfWeek> daysOff, HourWindow workHours,
        Dictionary<DayOfWeek, List<DayOffNeed>> needsByDay, CityEmploymentContext context) {
        ...
        RegisterSoloDayOff(adultId, context.HomeRoomIdByMemberId[adultId], daysOff, workHours, needsByDay);
    }
    ...
    private static void RegisterFamilyDay(Creature father, Creature homemaker, IReadOnlyList<Creature> kids,
        IReadOnlyList<DayOfWeek> daysOff, HourWindow workHours, Dictionary<DayOfWeek, List<DayOffNeed>> needsByDay,
        ...
        var need = new DayOffNeed(participantIds, homeRoomId, action, workHours, excludeTavern, false);
    ...
    private static void RegisterSoloDayOff(Guid creatureId, Guid homeRoomId, IReadOnlyList<DayOfWeek> daysOff,
        HourWindow workHours, Dictionary<DayOfWeek, List<DayOffNeed>> needsByDay) {
        var action = DayOffActivities[Random.Shared.Next(DayOffActivities.Length)];
    ...
    private static void RegisterUnemployedWeek(Creature adult, Dictionary<DayOfWeek, List<DayOffNeed>> needsByDay,
        CityEmploymentContext context) {
        var homeRoomId = context.HomeRoomIdByMemberId[adult.Id];

        foreach (var day in AllWeekdays) {
            var action = DayOffActivities[Random.Shared.Next(DayOffActivities.Length)];
            needsByDay[day].Add(new DayOffNeed([adult.Id], homeRoomId, action, new HourWindow(6, 22), false, true));
        }
    }
    ...
    private static void GenerateDayOffJobs(Dictionary<DayOfWeek, List<DayOffNeed>> needsByDay,
        CityEmploymentContext context) {
        foreach (var (day, needs) in needsByDay) {
            ...
            foreach (var need in needs) {
                ...
                foreach (var participantId in need.ParticipantIds) {
                    context.Jobs.Add(need.IsUnemployedActivity
                        ? JobGenerator.GenerateUnemployedDayActivity(context.StateId, participantId, need.Action,
                            roomId, day, context.WorldId)
                        : JobGenerator.GenerateDayOff(context.StateId, participantId, need.Action, roomId, day,
                            context.WorldId, need.Hours));
                }
            }
        }
    }
    ...
    private static Guid? PickDayOffRoom(DayOffNeed need, IReadOnlyList<IdleCandidate> cityIdleCandidates,
        List<int> remainingCapacity) {
        var eligibleIndices = Enumerable.Range(0, cityIdleCandidates.Count)
            .Where(i => remainingCapacity[i] >= need.ParticipantIds.Count)
            .Where(i => !need.ExcludeTavern || cityIdleCandidates[i].BuildingType != BuildingType.Tavern)
            .ToArray();
    ...
    internal record IdleCandidate(Guid? RoomId, Guid DistrictId, int Capacity, int Weight, BuildingType? BuildingType);
