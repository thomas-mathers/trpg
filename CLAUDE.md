...
### Query tracking
- Public **query methods** (read-only, never call `SaveChangesAsync`) use `.AsNoTracking()` on their queries — e.g. `LocationService`, `CreatureKnowledgeService`, `CreatureService.GetAllInState`/`GetByName*`, `SceneService`'s `districts`/`props` fetches
...
- The rule that makes this safe: a command method **never returns the tracked entity it mutated** to its caller. Its effect is observed either by calling code re-querying (a fresh query always reflects what was just saved, tracked or not) or by the command method returning a plain value describing the result (see `BuildingService.SetFrontDoorLocked` / `LockService.SyncScheduleLock`, which return the resulting `bool?` lock state rather than the `Prop`)
...
- Watch for *cross-service* ordering/identity assumptions when two methods touch the same row through separate queries on the same `DbContext`: `SceneService.GetScene` used to read a room's front door's `IsLocked` off an already-fetched `Prop` list *after* calling `lockService.SyncScheduleLock` (which mutates that same row through `BuildingService.SetFrontDoorLocked`'s own separate query) — this only worked because both queries were tracked and EF's identity map handed back the same in-memory object. It's fixed now by having `SetFrontDoorLocked`/`SyncScheduleLock` return the resulting `bool?` lock state directly, so `SceneService` no longer depends on the identity map to observe the mutation — but it's the one place in the codebase that shape existed, so check for it (something holding a pre-fetched entity/list across a call to another method that independently re-fetches-mutates-saves that same row, then reading the original reference afterward) before assuming a new cross-service interaction is safe
