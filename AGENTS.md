...
- **Scene refresh**: a GameTurns workflow or host adapter → GameTurns' `RefreshSceneCommand` → LocationSimulation's `CatchUpLocationCommand` advances time-dependent location state at most once per in-game hour, including route-backed creature relocation; worker job and occupancy effects run only after arrival → GameTurns' `GetSceneQuery` composes the current scene projection, including compact traveler journey context for the LLM.
