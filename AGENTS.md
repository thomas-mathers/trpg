...
- **Scene refresh**: a GameTurns workflow or host adapter → GameTurns' `RefreshSceneCommand` → LocationSimulation's `CatchUpLocationCommand` advances time-dependent location state at most once per in-game hour, including route-backed guard patrol and road-traveler relocation → GameTurns' `GetSceneQuery` composes the current scene projection, including compact road-traveler journey context for the LLM.
