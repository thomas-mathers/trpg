...
- **Scene refresh**: a GameTurns workflow or host adapter → GameTurns' `RefreshSceneCommand` → LocationSimulation's `CatchUpLocationCommand` advances time-dependent location state at most once per in-game hour. Catch-up ensures the job-derived recurring route projection exists, materializes any scheduled journey currently passing through the location, then relocates route-backed creatures; worker job and occupancy effects run only after arrival → GameTurns' `GetSceneQuery` composes the current scene projection, including compact traveler journey context for the LLM.
