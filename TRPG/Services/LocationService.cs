...
internal class LocationService(TrpgDbContext context, IMemoryCache cache)
{
    ...
    public async Task<IReadOnlyCollection<Country>> GetAllCountriesByWorldId(
        Guid worldId,
        CancellationToken cancellationToken = default
    )
    {
    ...
    public async Task<IReadOnlyCollection<State>> GetAllStatesByCountryId(
        Guid countryId,
        CancellationToken cancellationToken = default
    )
    {
    ...
    public async Task<IReadOnlyCollection<District>> GetAllDistrictsByCityId(
        Guid cityId,
        CancellationToken cancellationToken = default
    )
    {
    ...
    public async Task<District?> GetDistrictByNameInCity(
        Guid cityId,
        string name,
        CancellationToken cancellationToken = default
    )
    {
        return await context
            .Districts.AsNoTracking()
            .FirstOrDefaultAsync(
                d => d.CityId == cityId && EF.Functions.ILike(d.Name, name),
                cancellationToken
            );
    }
}
