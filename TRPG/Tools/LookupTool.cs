...
internal class LookupTool : Tool, IInvokableTool {
    ...
    public LookupTool(GameSession session, CreatureService creatureService,
        CreatureKnowledgeService creatureKnowledgeService, ILogger<LookupTool> logger) {
        _session = session;
        ...
        Function = new Function {
            Name = "lookup",
            Description =
                "Returns background information about a named country, city, faction, or person anywhere in the world — automatically figures out which kind of thing the name refers to, so you don't need to know its category in advance. This represents what a specific NPC knows or has heard, not omniscient narrator knowledge — call it on behalf of the NPC who would be recalling or sharing this information in dialogue, never to generate the narrator's own scene-setting exposition. Call this whenever dialogue turns to a specific person, place, or faction mentioned by name — a family member, acquaintance, distant place, or faction, including the NPC you're currently speaking with when asked about a specific detail (like family, hometown, or a faction tie) that start_conversation's biography didn't already cover — rather than inventing detail that isn't grounded in a tool result. Never state a specific personal-history fact — a relative, a death, a past event — unless it came from this tool's result or start_conversation's biography; if you don't have it, call lookup before answering rather than making one up. Returns an Error if the name isn't recognized as any of these, or if the speaker wouldn't know about it.",
    ...
    public object? InvokeMethod(IDictionary<string, object?>? args) {
        if (args is null || !args.TryGetValue("askingPersonName", out var askingPersonNameRaw) ||
            string.IsNullOrWhiteSpace(askingPersonNameRaw?.ToString())) {
            return new { Error = "No askingPersonName provided." };
        }

        if (!args.TryGetValue("subjectName", out var subjectNameRaw) ||
        ...
        _logger.LogInformation("[lookup] askingPersonName={AskingPersonName} subjectName={SubjectName}",
            askingPersonName, subjectName);
        var result = InvokeMethodAsync(askingPersonName, subjectName, CancellationToken.None).GetAwaiter()
            .GetResult();
    ...
    private async Task<object?> InvokeMethodAsync(string askingPersonName, string subjectName,
        CancellationToken cancellationToken) {
        var player = await _creatureService.GetById(_session.PlayerId, cancellationToken);
        var askingPerson =
            await _creatureService.GetByNameNearby(_session.WorldId, player!, askingPersonName, cancellationToken);
        if (askingPerson == null) {
            return new { Error = $"No one named '{askingPersonName}' found nearby. Call look to see who's around." };
        }
