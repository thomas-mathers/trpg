...
internal class GeographyGenerator(
    ...
    public async Task<GeographyGeneratorResult> Generate(
        ...
        var roads = GenerateRoadEntities(new GenerateRoadsInput(context, world, states, countries.DominantRaceByCountryId));
    ...
    private static readonly DistrictType[] RequiredDistrictTypes = [DistrictType.Residential, DistrictType.CityCenter];

    private static readonly DistrictType[] OptionalDistrictTypes = [
        DistrictType.Scientific, DistrictType.Governmental, DistrictType.HolySite, DistrictType.Encampment
    ];
    ...
    private static List<DistrictType> SelectDistrictTypes(bool isCapital, CountryFocus focus) {
        ...
        foreach (var districtType in OptionalDistrictTypes) {
            ...
            var chance = districtType == focusedDistrictType ? FocusedDistrictChance : OptionalDistrictChance;
        ...
        foreach (var (countryLayoutId, country) in countries.CountryById) {
            ...
            if (cityStates.Count > 0) {
                var cityNames = cityStates
                    .Select(_ => SettlementNameGenerator.GenerateCityName(dominantRace, context.ExistingNames))
                    .ToList();
                ...
                for (var chunkStart = 0; chunkStart < cityStates.Count; chunkStart += MaxCitiesPerRequest) {
                    ...
                    var chunkDistrictTypes = cityDistrictTypes.Skip(chunkStart).Take(MaxCitiesPerRequest).ToList();
                    ...
                    var schema = await client.GetJson<CityDescriptionListSchema>(
                        ...
                        $"""
                         You are a creative world-building assistant for a TRPG game generating content for: {context.GeneratorInput.Description}.
                         The world is {world.Name}: {world.Description}.
                         Respond with a JSON object with a Cities array containing exactly {chunk.Count} entries for the country of {country.Name}: {country.Description}, whose people are predominantly {dominantRace} and whose society is oriented around {FocusDescriptions[country.Focus]}.
                        ...
                        s => s.Cities.Count != chunk.Count
                            ? $"Expected exactly {chunk.Count} entries but got {s.Cities.Count}. You MUST produce one entry per city number."
                            : null,
                        cancellationToken);

                    var descriptionByIndex = schema.Cities.ToDictionary(c => c.Index, c => c.Description);

                    for (var j = 0; j < chunk.Count; j++) {
                        var mapState = chunk[j];
                        var cityName = chunkNames[j];
                        var description = descriptionByIndex.GetValueOrDefault(j + 1,
                        ...
                        foreach (var districtType in chunkDistrictTypes[j]) {
                            districts.Add(DistrictGenerator.Generate(districtType, city.Id, world.Id));
                        }
                    }
                }
            }
    ...
    private static List<Road> GenerateRoadEntities(GenerateRoadsInput input) {
        ...
        return context.Map.Roads.Select(layoutRoad => {
            ...
            var dominantRace = input.DominantRaceByCountryId.GetValueOrDefault(originState.CountryId, CreatureType.Human);
            return new Road {
