...
- **Scene refresh**: a GameTurns workflow or host adapter → GameTurns' `RefreshSceneCommand` → LocationSimulation's `CatchUpLocationCommand` advances time-dependent location state at most once per in-game hour → GameTurns' `GetSceneQuery` composes the current scene projection.
- **Quest seeding**: `CatchUpLocationCommand` calls LocationSimulation's `SyncQuestSeedScheduleCommand` at each `QuestSeedSchedule`d location (city entrances); on a due, chance-rolled tick it tries seven independent seed commands, each a no-op if nothing eligible exists. Six build and persist a single quest synchronously. The seventh, `SeedLlmQuestChainCommand`, only gathers a bounded real-entity pool and enqueues `GenerateQuestChainCommand` via the host-implemented `IQuestChainGenerationScheduler` (a TickerQ job, `api/TRPG/Quests/Jobs/GenerateQuestChainJob.cs`) — LLM-authored chain generation has been measured at 50-70 seconds and must never run inline. That job calls WorldGeneration's `QuestChainGenerator` (schema-validated: cycle-free, every entity id real and of the expected kind, every objective type's required fields present) to author a branching/converging DAG of quests in one pass, then persists the whole graph via `Quest.PrerequisiteQuestIds`. A `QuestChainGenerationRequest` row tracks Pending/InProgress/Completed/Failed so a request is never left stuck and a second seed roll doesn't pile up a duplicate job.
