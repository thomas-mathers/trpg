...
internal static class BuildingTemplate {
    private static readonly BuildingType[] StandardTypes = [
    ...
    private static readonly BuildingType[] CapitalOnlyTypes = [
        BuildingType.Castle,
        BuildingType.Jail
    ];
    ...
    public static IReadOnlyList<Building> GenerateForCity(City city, int housesPerCity) {
        var types = city.IsCapital ? [..StandardTypes, ..CapitalOnlyTypes] : StandardTypes;
        var buildings = new List<Building>(types.Length + housesPerCity);

        buildings.AddRange(types.Select(type => new Building {
            CityId = city.Id,
            BuildingType = type,
            Name = Names.TryGetValue(type, out var names) ? names[Random.Shared.Next(names.Length)] : type.ToString(),
            Boundary = new Rectangle(0, 0, 0, 0)
        }));

        for (var i = 1; i <= housesPerCity; i++) {
    ...
    public static BuildingContent Create(
        ...
        return new BuildingContent(rooms, props);
    }
    ...
    private static RoomSpec[] GetSpecs(BuildingType buildingType, Guid ownerId) =>
        buildingType switch {
            ...
            BuildingType.Jail => GetJailSpecs(ownerId),
            _ => []
        };
    ...
    private static RoomSpec[] GetBlacksmithSpecs(Guid ownerId) => [
        ...
        new RoomSpec("Living Quarters", "Simple living quarters above the shop.", 1, [
            new PropSpec("Bed",
                id => new Bed { RoomId = id, Name = "Bed", Description = "A modest bed.", AssignedPersonId = ownerId }),
            new PropSpec("Chest",
                id => new Container { RoomId = id, Name = "Chest", Description = "A sturdy chest for personal belongings." })
        ])
    ];
    ...
    private static RoomSpec[] GetTempleSpecs(Guid ownerId) => [
        ...
        new RoomSpec("Quarters", "Living quarters for the temple keeper.", 1, [
            new PropSpec("Bed",
                id => new Bed { RoomId = id, Name = "Bed", Description = "A simple bed.", AssignedPersonId = ownerId }),
            new PropSpec("Chest",
                id => new Container { RoomId = id, Name = "Chest", Description = "A chest for personal belongings." })
        ])
    ];
    ...
    private static RoomSpec[] GetTavernSpecs(Guid ownerId) => [
        ...
        new RoomSpec("Guest Room 1", "A small but tidy guest room.", 1, [
            new PropSpec("Bed", id => new Bed { RoomId = id, Name = "Bed", Description = "A comfortable bed for guests." }),
            new PropSpec("Chest", id => new Container { RoomId = id, Name = "Chest", Description = "A chest for guest belongings." })
        ]),
        ...
        new RoomSpec("Owner's Quarters", "The owner's private living space.", 2, [
            new PropSpec("Bed",
                id => new Bed { RoomId = id, Name = "Bed", Description = "The owner's bed.", AssignedPersonId = ownerId }),
            new PropSpec("Chest",
                id => new Container { RoomId = id, Name = "Chest", Description = "A chest for the owner's belongings." })
        ])
    ];
    ...
    private static RoomSpec[] GetGuildHallSpecs(Guid ownerId) => [
        ...
        new RoomSpec("Office", "A private office for the guild master.", 1, [
            new PropSpec("Bed",
                id => new Bed { RoomId = id, Name = "Bed", Description = "The guild master's bed.", AssignedPersonId = ownerId }),
            new PropSpec("Desk",
                id => new Workstation {
    ...
    private static RoomSpec[] GetCastleSpecs(Guid ownerId) => [
        new RoomSpec("Great Hall", "A grand hall fit for a lord.", 0, [
            new PropSpec("Throne", id => new Seat { RoomId = id, Name = "Throne", Description = "An imposing throne." }),
            new PropSpec("Chair", id => new Seat { RoomId = id, Name = "Chair", Description = "A chair for guests." }),
            new PropSpec("Chair", id => new Seat { RoomId = id, Name = "Chair", Description = "A chair for guests." }),
            new PropSpec("Chair", id => new Seat { RoomId = id, Name = "Chair", Description = "A chair for guests." }),
            new PropSpec("Counter",
                id => new Workstation {
        ...
        new RoomSpec("Royal Chambers", "Lavish private chambers for the lord.", 1, [
            new PropSpec("Bed",
                id => new Bed { RoomId = id, Name = "Bed", Description = "A grand four-poster bed.", AssignedPersonId = ownerId }),
            new PropSpec("Chest",
                id => new Container { RoomId = id, Name = "Chest", Description = "An ornate chest." })
        ])
    ];

    private static RoomSpec[] GetJailSpecs(Guid ownerId) => [
        ...
        new RoomSpec("Cells", "A row of dark, damp cells.", 1, [
            new PropSpec("Chest",
                id => new Container { RoomId = id, Name = "Chest", Description = "A chest for confiscated belongings." })
        ])
    ];
...
internal record BuildingContent(IReadOnlyList<Room> Rooms, IReadOnlyList<Prop> Props);