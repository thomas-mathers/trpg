...
internal class AccessoryGenerator {
    ...
    private static readonly ModifierTemplate[] Modifiers = [
        ...
        new(1, ModifierKey.IceResistance, 70,
            level => new AttributeModifier
                { Attribute = AttributeName.IceResistance, Type = AmountType.Percent, Amount = Roll(level, 5, 30) }),
        new(1, ModifierKey.LightningResistance, 70,
            level => new AttributeModifier {
                Attribute = AttributeName.LightningResistance, Type = AmountType.Percent, Amount = Roll(level, 5, 30)
            }),
        new(5, ModifierKey.PoisonResistance, 50,
            level => new AttributeModifier
                { Attribute = AttributeName.PoisonResistance, Type = AmountType.Percent, Amount = Roll(level, 5, 30) }),
        new(5, ModifierKey.MagicResistance, 20,
            level => new AttributeModifier
                { Attribute = AttributeName.MagicResistance, Type = AmountType.Percent, Amount = Roll(level, 5, 25) }),
        new(10, ModifierKey.SkillBonus, 15,
            _ => new SkillBonusModifier { Skill = null, Amount = 1 }),
        new(20, ModifierKey.FasterCastRate, 10,
