...
internal class CreatureService(TrpgDbContext context)
{
    ...
    public async Task<IReadOnlyCollection<Creature>> GetAllInState(
        Guid worldId,
        Guid stateId,
    ...
    public async Task<IReadOnlyCollection<CreatureSummary>> GetAllNearby(
        CreatureLocation location,
        Guid? excludingCreatureId = null,
        CancellationToken cancellationToken = default
    )
    {
    ...
    public async Task<IReadOnlyList<Guid>> GetIdsByDistrict(
        Guid worldId,
        Guid districtId,
        CancellationToken cancellationToken = default
    )
    {
        var ids = await context
            .Creatures.Where(p => p.WorldId == worldId && p.DistrictId == districtId)
            .Select(p => p.Id)
            .ToArrayAsync(cancellationToken);
        return ids;
    }

    public async Task<Creature?> GetByNameInRoom(
        Guid worldId,
        Guid roomId,
        string name,
        CancellationToken cancellationToken = default
    )
    {
        return await context
            .Creatures.AsNoTracking()
            .FirstOrDefaultAsync(
                p => p.WorldId == worldId && p.RoomId == roomId && p.Name == name,
                cancellationToken
            );
    }

    public async Task<Creature?> GetByNameOutdoorsInState(
        Guid worldId,
        Guid stateId,
        string name,
        CancellationToken cancellationToken = default
    ...
    public async Task<Creature?> GetByNameOutdoorsInDistrict(
        Guid worldId,
        Guid districtId,
        string name,
        CancellationToken cancellationToken = default
    ...
    public async Task<Creature?> GetByNameNearby(
        Guid worldId,
        Creature player,
        string name,
        CancellationToken cancellationToken = default
