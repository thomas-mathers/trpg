    ...
    protected override string BuildNarrationPrompt(
        ...
        action switch
        {
            FleeTheftEncounterAction =>
                $"The player chose to flee after {resolution.ConfrontingName} caught them stealing. Result: {JsonSerializer.Serialize(resolution, TRPG.Application.Common.Serialization.TrpgJsonOptions.Default)}. Narrate them wrenching free and escaping before {resolution.ConfrontingName} can stop them — no violence occurs. The result states whether they still hold the items: when itemsHeldByPlayer is false they escape empty-handed and the items never left their owner. Do not call any tools.",
