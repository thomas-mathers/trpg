...
internal class BuildingService(TrpgDbContext context, IMemoryCache cache) {
    ...
    public async Task<Building?> GetById(Guid id, CancellationToken cancellationToken = default) {
        return await cache.GetOrCreateAsync($"building:{id}",
            _ => context.Buildings.AsNoTracking().FirstOrDefaultAsync(b => b.Id == id, cancellationToken));
    }
    
    public async Task<RoomSummary?> GetRoomSummary(Guid roomId, CancellationToken cancellationToken = default) {
        return await cache.GetOrCreateAsync($"roomSummary:{roomId}", async _ => await (
            ...
            select new RoomSummary(r.Name, r.Description, r.FloorNumber, b.Id, b.Name, b.BuildingType,
                owner != null ? owner.Name : null, f != null ? f.Name : null, f != null ? f.Description : null)
    ...
    public async Task<IReadOnlyDictionary<Guid, Room>> GetRoomsByIds(IReadOnlyCollection<Guid> roomIds,
        CancellationToken cancellationToken = default) {
        ...
        if (missingIds.Count > 0) {
            var fetched = await context.Rooms.AsNoTracking()
    ...
    public async Task<IReadOnlyCollection<Prop>> GetStaticPropsByRoomId(Guid roomId,
        CancellationToken cancellationToken = default) {
        var props = await cache.GetOrCreateAsync<Prop[]>($"staticProps:{roomId}",
            async _ => (await context.Props.AsNoTracking()
                .Where(p => p.RoomId == roomId)
                .ToArrayAsync(cancellationToken))
    ...
    public async Task<IReadOnlyCollection<RoomConnector>> GetConnectorsByRoomId(Guid roomId,
        CancellationToken cancellationToken = default) {
    ...
    public async Task<IReadOnlyCollection<Building>> GetAllByLocation(Guid stateId, Guid? cityId, Guid? districtId,
        CancellationToken cancellationToken = default) {
        var buildings = await cache.GetOrCreateAsync<Building[]>($"nearbyBuildings:{stateId}:{cityId}:{districtId}",
            async _ => await context.Buildings.AsNoTracking()
                .Where(b => b.StateId == stateId && b.CityId == cityId && b.DistrictId == districtId)
                .ToArrayAsync(cancellationToken));
        return buildings ?? [];
    }
