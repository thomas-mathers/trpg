...
)
{
    ...
    public async Task<bool> SyncIfNeeded(
        GameSession session,
        Guid worldId,
        Guid? roomId,
        Guid? districtId,
    ...
    {
        var creatureIds = await creatureService.GetIdsByDistrict(
            worldId,
            districtId,
            cancellationToken
        );
    ...
    )
    {
        ...
        await SyncScheduleLock(
            roomSummary.BuildingId,
    ...
    public async Task<bool?> SyncScheduleLock(
        Guid buildingId,
    ...
    )
    {
        ...
        return await buildingService.SetFrontDoorLocked(
            buildingId,
            activeJob.Action == JobAction.Sleep,
            cancellationToken
        );
    }
}
