...
public sealed class SessionEndpointsTests(EndpointTestFixture fixture) : IAsyncLifetime {
    ...
    public async Task SendChat_ReturnsNarration_WhenSessionExists() {
        ...
        // Act
        var response = await _client.PostAsJsonAsync(new Uri($"/sessions/{sessionId}/chat", UriKind.Relative),
            new ChatRequest("look around"), TestContext.Current.CancellationToken);

        // Assert
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        var result = await response.Content.ReadFromJsonAsync<ChatResponse>(TestContext.Current.CancellationToken);
        Assert.NotNull(result);
    ...
    public async Task Wait_AdvancesTimeAndReturnsMessage_WhenSessionExists() {
        ...
        // Act
        var response = await _client.PostAsJsonAsync(new Uri($"/sessions/{sessionId}/wait", UriKind.Relative),
            new WaitRequest(5), TestContext.Current.CancellationToken);
    ...
    public async Task EndSession_SavesPlaytimeAndReturnsNoContent_WhenSessionExists() {
        ...
        // Act
        var response = await _client.DeleteAsync(new Uri($"/sessions/{sessionId}", UriKind.Relative),
            TestContext.Current.CancellationToken);
        ...
        await using var scope = fixture.CreateScope();
        var context = scope.ServiceProvider.GetRequiredService<TrpgDbContext>();
        var world = await context.Worlds.FindAsync([_worldId], TestContext.Current.CancellationToken);
    ...
    public async Task StartSession_ReturnsNotFound_WhenWorldHasNoPlayer() {
        ...
        // Act
        var response = await _client.PostAsync(new Uri($"/worlds/{world.Id}/sessions", UriKind.Relative), null,
            TestContext.Current.CancellationToken);
    ...
    public async Task SendChat_ReturnsNotFound_WhenSessionDoesNotExist() {
        // Act
        var response = await _client.PostAsJsonAsync(new Uri($"/sessions/{Guid.NewGuid()}/chat", UriKind.Relative),
            new ChatRequest("look around"), TestContext.Current.CancellationToken);
    ...
    public async Task Wait_ReturnsNotFound_WhenSessionDoesNotExist() {
        // Act
        var response = await _client.PostAsJsonAsync(new Uri($"/sessions/{Guid.NewGuid()}/wait", UriKind.Relative),
            new WaitRequest(5), TestContext.Current.CancellationToken);
    ...
    public async Task Wait_ReturnsBadRequest_WhenHoursIsNotPositive() {
        ...
        // Act
        var response = await _client.PostAsJsonAsync(new Uri($"/sessions/{sessionId}/wait", UriKind.Relative),
            new WaitRequest(0), TestContext.Current.CancellationToken);
    ...
    public async Task EndSession_ReturnsNotFound_WhenSessionDoesNotExist() {
        // Act
        var response = await _client.DeleteAsync(new Uri($"/sessions/{Guid.NewGuid()}", UriKind.Relative),
            TestContext.Current.CancellationToken);
