...
public sealed class SceneSyncServiceTests(DatabaseFixture db) : IAsyncLifetime {
    ...
    public async ValueTask InitializeAsync() {
        ...
        var dispatcher = new JobDispatcher(
            ...
            new IdleJobHandler(_creatureService), new StudyJobHandler(_creatureService),
            new PrayJobHandler(_creatureService), new TrainJobHandler(_creatureService),
            new SitJobHandler(_creatureService), NullLogger<JobDispatcher>.Instance);
    ...
    private static InGameDate MakeDate(int hour) => new(975, "Thawmoon", 1, "Stormday", DayOfWeek.Thursday, hour);
    ...
    public async Task SyncIfNeeded_MovesCreatureIntoRoom_WhenSleepJobActive() {
        ...
        await _service.SyncIfNeeded(session, _worldId, sleepRoomId, null, MakeDate(23),
    ...
    public async Task SyncIfNeeded_MovesCreatureOut_WhenHigherPriorityWorkJobActiveElsewhere() {
        ...
        await _service.SyncIfNeeded(session, _worldId, sleepRoomId, null, MakeDate(10),
            TestContext.Current.CancellationToken);
    ...
    public async Task SyncIfNeeded_DoesNothing_WhenNoJobsTargetRoom() {
        ...
        // Act
        await _service.SyncIfNeeded(session, _worldId, Guid.NewGuid(), null, MakeDate(12),
            TestContext.Current.CancellationToken);
    ...
    public async Task SyncIfNeeded_MovesCreatureOutdoors_WhenIdleJobActive() {
        ...
        await _service.SyncIfNeeded(session, _worldId, null, districtId, MakeDate(12),
            TestContext.Current.CancellationToken);
    ...
    public async Task SyncIfNeeded_AssignsBothWorkersToDifferentWorkstations_WhenTwoArePresent() {
        ...
        var counter = new Workstation {
            RoomId = shopRoomId, WorldId = _worldId, Name = "Counter", Description = "A counter.",
            WorkstationType = WorkstationType.Trade
        };
        var oven = new Workstation {
            RoomId = shopRoomId, WorldId = _worldId, Name = "Oven", Description = "An oven.",
        ...
        await _jobService.Add(Builders.MakeJob(owner.Id, action: JobAction.Work, startHour: 8, endHour: 20,
            roomId: shopRoomId, priority: 50), TestContext.Current.CancellationToken);
        await _jobService.Add(Builders.MakeJob(employee.Id, action: JobAction.Work, startHour: 8, endHour: 20,
            roomId: shopRoomId, priority: 50), TestContext.Current.CancellationToken);
        var session = new GameSession(_worldId, Guid.NewGuid(), TimeSpan.Zero);

        // Act
        await _service.SyncIfNeeded(session, _worldId, shopRoomId, null, MakeDate(12),
            TestContext.Current.CancellationToken);

        // Assert — both workstations get staffed, by two different people
        var workstations = await _buildingService.GetWorkstationsByRoomId(shopRoomId,
            TestContext.Current.CancellationToken);
    ...
    public async Task SyncIfNeeded_GivesCounterPriority_AndClearsUnstaffedStation_WhenOnlyOneWorkerPresent() {
        ...
        var oven = new Workstation {
            RoomId = shopRoomId, WorldId = _worldId, Name = "Oven", Description = "An oven.",
            WorkstationType = WorkstationType.Cooking, OccupantId = Guid.NewGuid() // stale, from a prior day
        };
        _context.Props.Add(counter);
        _context.Props.Add(oven);
        await _context.SaveChangesAsync(TestContext.Current.CancellationToken);
    ...
    public async Task SyncIfNeeded_LocksFrontDoor_WhenOwnerHasActiveSleepJob() {
        ...
        // Act — hour 23 falls inside the wraparound Sleep window
        await _service.SyncIfNeeded(session, _worldId, frontDoor.RoomId, null, MakeDate(23),
            TestContext.Current.CancellationToken);
    ...
    public async Task SyncScheduleLock_Locks_DuringSleepHours() {
        ...
        // Act
        await _service.SyncScheduleLock(building.Id, building.BuildingType, MakeDate(23),
            TestContext.Current.CancellationToken);
    ...
    public async Task SyncScheduleLock_Unlocks_DuringWorkHours() {
        ...
        // Act
        await _service.SyncScheduleLock(building.Id, building.BuildingType, MakeDate(12),
            TestContext.Current.CancellationToken);
    ...
    public async Task SyncScheduleLock_NeverLocks_InnOrTavern() {
        ...
        // Act
        await _service.SyncScheduleLock(building.Id, BuildingType.Tavern, MakeDate(23),
            TestContext.Current.CancellationToken);
    ...
    private async Task<RoomConnector> SeedFrontDoor(Guid buildingId) {
        ...
        var frontDoor = new RoomConnector {
            RoomId = entranceRoom.Id, WorldId = _worldId, Name = "Front Door",
    ...
    public async Task SyncIfNeeded_ReturnsFalse_WhenScopeAndDateUnchanged() {
        ...
        await _service.SyncIfNeeded(session, _worldId, roomId, null, currentDate,
            TestContext.Current.CancellationToken);

        // Act
        var syncedAgain = await _service.SyncIfNeeded(session, _worldId, roomId, null, currentDate,
            TestContext.Current.CancellationToken);
