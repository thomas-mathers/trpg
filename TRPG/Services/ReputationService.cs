...
internal class ReputationService(TrpgDbContext context) {
    ...
    public async Task<int> GetEffectiveReputation(Guid observerCreatureId, Guid targetCreatureId,
        ...
        var reputations = await GetEffectiveReputations(observerCreatureId, [targetCreatureId], factionIdsByCreature,
            cancellationToken);
        return reputations.GetValueOrDefault(targetCreatureId, 0);
    }

    public async Task<IReadOnlyDictionary<Guid, int>> GetEffectiveReputations(Guid observerCreatureId,
        IReadOnlyCollection<Guid> targetCreatureIds, IReadOnlyDictionary<Guid, Guid[]> factionIdsByCreature,
        CancellationToken cancellationToken = default) {
        ...
        var reputations = await context.Reputations
            .Where(r => r.CreatureId == observerCreatureId &&
                        ((r.TargetType == ReputationTargetType.Faction && allFactionIds.Contains(r.TargetId)) ||
                         (r.TargetType == ReputationTargetType.Creature && targetCreatureIds.Contains(r.TargetId))))
            .ToArrayAsync(cancellationToken);
        ...
        return targetCreatureIds.ToDictionary(id => id, id => {
            var directScore = reputationByCreatureId.GetValueOrDefault(id, 0);
            var factionScore = factionIdsByCreature.GetValueOrDefault(id, [])
