...
public class WorldGeneratorHouseholdTests {
    ...
    private readonly City _city = new() { WorldId = Guid.NewGuid(), StateId = Guid.NewGuid(), Name = "Test City" };
    private readonly District _residentialDistrict = new() {
        WorldId = Guid.NewGuid(), CityId = Guid.NewGuid(), Name = "Old Town", DistrictType = DistrictType.Residential
    };
    private readonly WorldGeneratorInput _generatorInput = new() {
        Description = "test", MinHouseholdSize = 1, MaxHouseholdSize = 4
    };
    ...
    private static WorldGenerator MakeWorldGenerator() {
        ...
        var itemGenerator = new ItemGenerator(
            new WeaponGenerator(abilityDefinitions), new ArmorGenerator(abilityDefinitions),
            new AccessoryGenerator(), new ConsumableGenerator(), new AmmoGenerator());
        var creatureGenerator = new CreatureGenerator(itemGenerator, abilityDefinitions);
        return new WorldGenerator(new BuildingGenerator(), null!, null!, creatureGenerator,
    ...
    private WorldGenerator.HouseholdGenerationContext MakeContext() {
        return new WorldGenerator.HouseholdGenerationContext {
            WorldId = _worldId, City = _city, StateId = _stateId, ResidentialDistrict = _residentialDistrict,
            DominantRace = CreatureType.Human, GeneratorInput = _generatorInput, UsedBuildingNames = [],
            CityFactionId = Guid.NewGuid(), CityBuildings = [], Creatures = [], FactionMembers = [], Items = [],
            InventoryItems = [], Skills = [], Abilities = [], Jobs = [], BuildingOwners = [], Rooms = [], Props = [],
            Relationships = [], RoomConnectorKeys = [], HouseholdByMemberId = [], HomeRoomIdByMemberId = [],
            FatherIds = [], EligibleForEmployment = []
        };
    }
    ...
    public void GenerateSingleHousehold_AlwaysProducesAnAdult() {
        for (var i = 0; i < 50; i++) {
            // Act
            var household = _worldGenerator.GenerateSingleHousehold(CreatureType.Human, _worldId, _stateId);

            // Assert
            var member = Assert.Single(household.Members);
            Assert.True(EpochYear - member.Creature.BirthYear >= 18,
    ...
    public void GenerateFamilyHousehold_AlwaysProducesAdultParentsWithASpouseRelationship() {
        // Act
        var household = _worldGenerator.GenerateFamilyHousehold(CreatureType.Human, _worldId, _stateId,
            _generatorInput);
        ...
        Assert.Contains(household.Relationships,
            r => r.SubjectId == mother.Creature.Id && r.RelativeId == father.Creature.Id &&
                r.RelationshipType == RelationshipType.Husband);
        Assert.Contains(household.Relationships,
            r => r.SubjectId == father.Creature.Id && r.RelativeId == mother.Creature.Id &&
    ...
    public void GenerateAndRegisterHousehold_AssignsDesignatedProfession_AndRegistersFullSchedule() {
        ...
        Assert.Contains(context.Jobs, j => j.CreatureId == worker.Id && j.Action == JobAction.Sleep);
    ...
    public void GenerateAndRegisterHousehold_GivesDesignatedWorkerASpouse_WhenHouseholdIsAFamily() {
        ...
        for (var attempt = 0; attempt < 30; attempt++) {
            ...
            // Assert
            Assert.Contains(context.Relationships,
                r => r.SubjectId == worker.Id &&
                    (r.RelationshipType == RelationshipType.Husband || r.RelationshipType == RelationshipType.Wife));
            return;
        }

        Assert.Fail("Never generated a family household in 30 attempts — FamilyUnitChance may have regressed.");
    }
    ...
    public void GenerateAndRegisterHousehold_NeverAssignsARealJob_WhenNoProfessionIsDesignated() {
        ...
        Assert.Null(worker);
        Assert.All(context.Creatures,
            c => Assert.True(c.Profession is null or Profession.Unemployed or Profession.Homemaker,
    ...
    public void GenerateAndRegisterHousehold_NeverMakesTheSoleHouseholderAMinor() {
        for (var i = 0; i < 30; i++) {
            ...
            Assert.True(EpochYear - soleResident.BirthYear >= 18,
